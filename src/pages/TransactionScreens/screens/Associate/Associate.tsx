...
import { PostAssociate, getAssociateRouteCad, getroutecardlist } from "./api";
...
import { Expired, getSessionToken } from "../../../../components/AuthUser";
...
const Associate = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "AssociateService");
  ...
  const handlepostsave = async (event) => {
    ...
    if (!!values.Routecard) {
      if (rowSelectionModel.length !== 0) {
        try {
          const response = await PostAssociate(body);
          if (response.data) {
      ...
      if (res.length === 0) {
        ...
        if (RouteCardId !== null || RouteCardId !== 0) {
         ...
         // setuomname(uomname);
          ...
          try {
            const response = await getAssociateRouteCad(body);

            if (response) {
              const res2 = response.data.selectedRouteCardID;
  ...
  const handleRowSelectionModelChange = (newSelection) => {
    const hasHoldStatus = newSelection.some(
      (id) => rows.find((row) => row.id === id)?.status === 3
    );
    if (hasHoldStatus) {
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        ...
        >
          ...
          >
            ...
            <h2 style={{ float: "right" }}>Associate</h2>
          </MuiModules.UIGrid>
        </MuiModules.UIGrid>
        ...
        <div className="subcontainer" style={{ marginTop: "20px" }}>
          <h5>PENDING ELIGIBLE ROUTECARDS:</h5>
          <DataGridPro
            rows={modifiedRows}
