...
import { PostMultirealse, getHoldCheck, getroutecardlist } from "./api";
...
const MultiRelease = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ReleaseMultipleService");
  ...
  const handlepostsave = async (event) => {
    ...
    if (!holdReason) {
      ...
      try {
        const response = await PostMultirealse(body);
        if (response.data) {
  ...
  const [ReleaseReason, setReleaseReason] = useState<string | null>("");
    ...
    >
      <form onSubmit={handleSubmit} onReset={handleReset}>
        ...
        >
          ...
          >
            <h2 style={{ float: "right" }}>Multi Release</h2>
          </MuiModules.UIGrid>
        </MuiModules.UIGrid>
          ...
          >
            ...
            >
              ...
              <MuiModules.UIAutocomplete
                disablePortal
                id="ReleaseReason"
                options={loadholdreasondata.map(
                  (item) => item.ReleaseReasonName
                )}
                renderInput={(params) => (
