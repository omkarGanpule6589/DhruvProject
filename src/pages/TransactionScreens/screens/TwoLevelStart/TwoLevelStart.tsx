...
import { ProductByProductionorder ,  TransactionStart,
  getCustomerNames,
  getFactoryNames,
  getLocationNames,
  getProcessflowNames,
  ...
  getUOMNames,
  getUnitLevelNames,
  getlocbyfac,} from "../StartTransaction/StartTransactionApi";
import { DataGridPro, GridColDef, GridRowSelectionModel } from "@mui/x-data-grid-pro";
import { GetChildRouteCards, Twolevelstarttransaction } from "./TwoLevelStartApi";
...
const TwoLevelStart = () => {
  const [Data, setData] = useState<RoutecardDetails[]>([]);
    ...
    const handleChildUnitLevel = (event, newValue) => {
      //setUnitLevelName(newValue);
      setFieldValue("ChildUnitLevelName", newValue);
  
      const selectedUnitLevel = UnitLevelData?.filter((ele) =>
        ele.UnitLevel1 === newValue ? ele.UnitLevelId : null
      );
      setFieldValue("ChildUnitLevelId", selectedUnitLevel?.[0]?.UnitLevelId ?? null);
    };
    ...
    const handleChildUom = (event, newValue) => {
      //setUOMName(newValue);
  
      const selectedUOM = UOMData?.filter((ele) =>
        ele.Uomname === newValue ? ele.Uomid : null
      );
      setFieldValue("ChildUOMId", selectedUOM?.[0]?.Uomid ?? null);
      setFieldValue("ChildUOMName", selectedUOM?.[0]?.Uomname ?? null);
    };
    ...
    const handleClearValues = () => {
      ...
      setFieldValue("ChildUnitLevelName",null);
      setFieldValue("ChildUnitLevelId", null);
      setFieldValue("ChildUOMId",null);
      setFieldValue("ChildUOMName",null);
      setFieldValue("ChildQty", "");
      setFieldValue("ChildcountQty","");
      setData([])
      
    };
  

    const handleAddChildRouteCards = async (event) => {
      setsubmitspinnerL(true);
      const body = {
        RouteCardStartDetails: [
          {
        ...
        TxnName:"SampleTransaction",
      }
    ],
    Qty: values.ChildQty,
  ChildCount:values.ChildcountQty,
  UnitLevel:values.ChildUnitLevelId,
  UOMId: values.ChildUOMId,
  UOMName:values.ChildUOMName,
  UnitLevelName:values.ChildUnitLevelName
      };
  
      try {
        
        const response = await GetChildRouteCards(body);
        if (response.data) {
          debugger
          const RoutecardDetailsWithIds = response.data.childRouteCards.map(item => ({
            ...item,
            RoutecardDetailId: `dummy-id-${Math.random().toString(36).substr(2, 9)}` // Generate a random ID
          }));
          setData(RoutecardDetailsWithIds)
  ...
  UOMId: values.ChildUOMId,
  UseNumberingRule: values.UseNumberingRule,
  childRouteCards: Data.map((row) => ({
    routeCard:row.routeCard,
    ...
    qty:row.qty    
      ...
      try {
        const response = await Twolevelstarttransaction(body);
        debugger
        if (response.data) {
          debugger
      ...
      >
        <form onSubmit={handleSubmit} onReset={handleReset}>
          ...
          >
            ...
            >
              <label style={{ fontSize: "14px" }}>
                Parent Level<span style={{ color: "red" }}>*</span>
              </label>
              <MuiModules.UIAutocomplete
                disablePortal
                id="Level"
...
<MuiModules.UIGrid
              ...
              style={{ display: "flex", flexDirection: "column" }}
            >
             <label style={{ fontSize: "14px" }}> Child Count</label>
            <MuiModules.UITextField
                  name="ChildcountQty"
                  id="ChildcountQty"
                   type="number"
                  value={values.ChildcountQty}
                  onChange={handleChange}
                  onBlur={handleBlur}
                  style={{ fontSize: "8px" }}
                  autoComplete="off"
          ...
          <MuiModules.UIGrid
              ...
              style={{ display: "flex", flexDirection: "column" }}
            >
              <label style={{ fontSize: "14px" }}> Child Qty</label>
              
            <MuiModules.UITextField
                  name="ChildQty"
                  id="ChildQty"
                  type="number"
                  value={values.ChildQty}
                  onChange={handleChange}
                  onBlur={handleBlur}
                  style={{ fontSize: "8px" }}
                  autoComplete="off"
          ...
          <MuiModules.UIGrid
              ...
              style={{ display: "flex", flexDirection: "column" }}
            >
              <label style={{ fontSize: "14px" }}> Child UOM</label>
              <MuiModules.UIAutocomplete
                disablePortal
                id="UOM"
                options={UOMData?.map((item) => item.Uomname)}
                ...
                onChange={(event, newValue) => {
                  handleChildUom(event, newValue);
                }}
                value={values.ChildUOMName}
              />
            </MuiModules.UIGrid>
            <MuiModules.UIGrid
              item
              ...
              style={{ display: "flex", flexDirection: "column" }}
            >
              <label style={{ fontSize: "14px" }}>Child Level</label>
              <MuiModules.UIAutocomplete
                disablePortal
                id="ChildLevel"
                options={UnitLevelData?.map((item) => item.UnitLevel1)}
                ...
                onChange={(event, newValue) => {
                  handleChildUnitLevel(event, newValue);
                }}
                value={values.ChildUnitLevelName}
              />
            ...
            <MuiModules.UIGrid
              ...
              sm={12}
              md={1}
              style={{ display: "flex", flexDirection: "column",marginTop:"30px",marginLeft:"10px" }}
            >
              <MuiModules.UIButton
               variant="contained"
               //color="primary"
               onClick={handleAddChildRouteCards} style={{ }}
             >
              Add
             </MuiModules.UIButton>
            </MuiModules.UIGrid>
            ...
            >
             <MuiModules.UIButton
               variant="contained"
               //color="primary"
               onClick={handleClearValues} style={{ }}
             >
              Clear
             </MuiModules.UIButton></MuiModules.UIGrid>
          </MuiModules.UIGrid>
          ...
          <div
           ...
           >
            <h4 > Child Route Cards  </h4>
            <GridPro 
              rows={Data}
              columns={columns}
              id="RoutecardDetailId"
