...
const Open = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "OpenService");
  ...
  const handlepostsave = async (event) => {
    ...
    if (!!values.Routecard) {
      try {
        const response = await postOpen(body);
        if (response.data) {
      ...
      if (res.length === 0) {
        ...
        if (RouteCardId !== null || RouteCardId !== 0) {
         ...
         // setuomname(uomname);
          ...
          try {
            const response = await getopentabout(body);
debugger
            if (response) {
          ...
          >
            ...
            >
              <label style={{ fontSize: "14px" }}>
                Change State Reason <span style={{ color: "red" }}>*</span>
              </label>
              <MuiModules.UIAutocomplete
