...
const DataCollectAccor1 = (props) => {
  ...
  const decimalRegex = /^(\d+(\.\d{1,4})?)?$/; // Allows 0 or any number with up to 2 decimal places
...
const handleChange1 = (index, e) => {
  ...
  if (row.dataPointType === "Decimal") {
    ...
    if (isValidDecimal(value)) {
      ...
      if (numericValue >= 0 && numericValue <= 9999999999999999999999999999999.99) {
      ...
      ErrorNotification("Invalid Decimal format. Please enter a valid decimal with up to 2 decimal places.");
    }
  }
    ...
    <div>
      <Accordion style={{ marginTop: "10px" }}>
        <AccordionSummary
          ...
          id="panel1-header"
        >
       <span>Data Collection - {sortedRows[0]?.dataCollectionName}</span>
        </AccordionSummary>
...
export default DataCollectAccor1;
