...
import { getRemovalDifferenceReasonList } from "./RemovalReasonAPI";
...
const RemoveDifferenceReason = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getRemovalDifferenceReasonList();
      setRemoveDifferenceReason(response.data.value);
      setError("");
    } catch (error) {
      setgridload(false);
  ...
  const baseColumns: GridColDef[] = [
    // { field: "RemoveDifferenceReasonId", headerName: "ID", width: 90 },
    {
      field: "RemoveDifferenceReasonName",
      headerName: "Remove Difference Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/RemoveDifferenceReason?key=${id}` });
      setDeleteDataName(params.row.RemoveDifferenceReasonName);
    },
    []
  );
  ...
  const handleAddClick = () => {
    navigate("/masterdata/removeDifferenceReasonAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/removeDifferenceReasonAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"RemoveDifferenceReason"} refresh={OnCallAPI} />
          {Add && (
        ...
        <GridPro
          rows={removeDifferenceReason}
          columns={columns}
          id="RemoveDifferenceReasonId"
...
export default RemoveDifferenceReason;
