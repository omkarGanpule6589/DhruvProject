...
import { validation } from "./RemoveDifferenceReasonValidation";
...
import {
  editRemovalReason,
  CreateRemovalReason,
  getRemoveDifferenceReasonById,
} from "./RemovalReasonAPI";
...
const RemoveDifferenceReasonAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.RemoveDifferenceReason
      ,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "RemoveDifferenceReason");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        ...
        try {
          const response = await getRemoveDifferenceReasonById(id);

          if (response.data) {
            const result = await response.data.value;
            const { RemoveDifferenceReasonName } = result[0] || {};
            initialValues.RemoveDifferenceReasonName =
              RemoveDifferenceReasonName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setError("");
            setorginalname(result[0].RemoveDifferenceReasonName);
  ...
  const handlePostRequest = async (event) => {
    ...
    const { RemoveDifferenceReasonName, Description } = values;
    const body = {
      MId: 1,
      RemoveDifferenceReasonName,
      Description,
    ...
    console.log(body);
    try {
      const response = await CreateRemovalReason(body);
      if (response.data) {
        setMsg(`${values.RemoveDifferenceReasonName} Created Successfully`);

        SuccessNotification(
          `Remove Difference Reason'${
            values.RemoveDifferenceReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/removedifferencereason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    try {
      const response = await editRemovalReason(id, body);
      if (response.data) {
        setMsg(`${values.RemoveDifferenceReasonName} Updated Successfully`);
        SuccessNotification(
          `Remove Difference Reason '${
            values.RemoveDifferenceReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).RemoveDifferenceReason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/removedifferencereason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id
              ? "Add Remove Difference Reason"
              : "Edit Remove Difference Reason"}
        ...
        >
          ...
          >
            <label htmlFor="RemoveDifferenceReasonName">
              Remove Difference Reason Name
              <span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="RemoveDifferenceReasonName"
            ...
            />
            {errors.RemoveDifferenceReasonName &&
            touched.RemoveDifferenceReasonName ? (
      ...
      {isDeleteCnfDialogOpen && (
        <ConfirmDialog
          ...
          data={deleteData}
          onDelete={OnCallAPI}
          screenName="Remove Difference Reason "
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="RemoveDifferenceReasonId"
          Bodyname="RemoveDifferenceReasonName"
        />
      )}
    </div>
  );
...
export default RemoveDifferenceReasonAddEdit;
