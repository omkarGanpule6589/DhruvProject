...
import { getWorkInstructionList } from "./WorkInstructionAPI";
...
function WorkInstruction() {
  ...
  const [data, setData] = useState<WorkInstructionList[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getWorkInstructionList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "WorkInstructionId", headerName: "ID", width: 90 },

    {
      field: "WorkInstructionName",
      headerName: "WorkInstruction Name",
      width: 200,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/WorkInstruction?key=${id}` });
      setDeleteDataName(params.row.WorkInstructionName);
    },
    []
  );
  ...
  const handleAddClick = () => {
    navigate("/masterdata/workInstructionAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/workInstructionAddEdit/${id}`);
    }
  };
  return (
    <div
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"WorkInstruction"} refresh={OnCallAPI} />
          {Add && (
