...
import { getProductionOrderStatusList } from "./ProductionOrderStatusApi";
...
const ProductionOrderStatus = () => {
  ...
  const [data, setData] = useState<ProductionOrderStatus[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getProductionOrderStatusList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "ProductionOrderStatusId", headerName: "ID", width: 100 },
    {
      field: "ProductionOrderStatusName",
      headerName: "Production Order Status Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ProductionOrderStatus?key=${id}` });
      setDeleteDataName(params.row.ProductionOrderStatusName);
    },
    []
  );

  const handleAddClick = () => {
    navigate("/masterdata/productionorderstatusaddedit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/productionorderstatusaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ProductionOrderStatus"} refresh={OnCallAPI} />
          {Add && (
