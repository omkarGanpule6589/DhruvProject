...
import {
  CreateProductionOrderStatus,
  EditProductionOrderStatusDetails,
  getProductionOrderStatuslDetailFetch,
} from "./ProductionOrderStatusApi";
...
const ProductionOrderStatusAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.ProductionOrderStatus,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ProductionOrderStatus");
  ...
  const validation12 = Yup.object({
    ProductionOrderStatusName: Yup.string()
      .trim()
      .required("Production Order Status Name is required"),
  });
  ...
  const fetchData = () => {
    if (id) {
      const fetchProductionOrderStatus = async () => {
        setformload(true);

        try {
          const response = await getProductionOrderStatuslDetailFetch(id);
          if (response.data.value.length > 0) {
            const result = response.data.value[0];
            (initialValues.ProductionOrderStatusName =
              result.ProductionOrderStatusName),
              (initialValues.Description = result.Description);
            setorginalname(result?.ProductionOrderStatusName);
  ...
  const handlePostRequest = async () => {
    ...
    try {
      const response = await CreateProductionOrderStatus(body);
      if (response.data) {
        setMsg(`${values.ProductionOrderStatusName} Created Successfully`);
        SuccessNotification(
          `Production Order Status '${
            values.ProductionOrderStatusName
          }' Created Successfully on '${cureenttime()}'`
        );

        setError(null);
        navigate("/masterdata/productionorderstatus");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await EditProductionOrderStatusDetails(id, values);
      if (response.data) {
        setMsg(`${values.ProductionOrderStatusName} Updated Successfully`);
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).ProductionOrderStatus  });
      ...
      >
        ...
        <form onSubmit={handleSubmit} onReset={handleReset}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <MuiIcons.ArrowCircleLeftOutlinedIcon
              onClick={() => navigate("/masterdata/productionorderstatus")}
              style={{ marginRight: "10px" }}
            ></MuiIcons.ArrowCircleLeftOutlinedIcon>
            <MuiModules.UITypography component="h1" variant="h5">
              {!id
                ? "Add Production Order Status "
                : "Edit Production Order Status"}
          ...
          >
            ...
            >
              <label htmlFor="ProductionOrderStatusName">
                Production Order Status Name{" "}
                <span style={{ color: "red" }}>*</span>
              </label>
              <MuiModules.UITextField
                name="ProductionOrderStatusName"
                id="ProductionOrderStatusName"
                value={values.ProductionOrderStatusName}
                ...
                autoComplete="off"
              />
              {errors.ProductionOrderStatusName &&
              touched.ProductionOrderStatusName ? (
        ...
        {isDeleteCnfDialogOpen && (
          <ConfirmDialog
            ...
            data={deleteData}
            onDelete={OnCallAPI}
            screenName="Production Order Status "
        ...
        {isCopyobjpopupOpen && (
          <ConfirmDialogCopyobj
            ...
            valueRev={copyobjrev}
            Bodyhead="ProductionOrderStatusId"
            Bodyname="ProductionOrderStatusName"
          />
        )}
      </div>
    </>
...
export default ProductionOrderStatusAddEdit;
