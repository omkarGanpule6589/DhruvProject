...
import { getSampletestlist } from "./SampleTestApi";
...
function SampleTest() {
  ...
  const [data, setData] = useState<SampleTestTypes[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getSampletestlist();
      const sortedData = response.data.value.sort((a, b) => {
        return a.SampleTestName.localeCompare(b.SampleTestName);
      });
  ...
  const baseColumns: GridColDef[] = [
    //  { field: "SampleTestId", headerName: "ID", width: 90 },
    {
      field: "SampleTestName",
      headerName: "Sample Test Name",
      width: 250,
    },
    {
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/SampleTest?key=${id}` });
      setDeleteDataName(params.row.SampleTestName);
    },
    []
  );
  const handleAddClick = () => {
    navigate("/masterdata/sampleTestAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/sampleTestAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: 400, width: "100%" }}>
        ...
        >
          <ExportImport Name={"SampleTest"} refresh={OnCallAPI} />
          {Add && (
