...
import { getProductTypeList } from "./ProductTypeAPI";
...
const ProductType = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setProductType(response.data.value);
      setError("");
    } catch (error) {
      setgridload(false);
  ...
  const baseColumns: GridColDef[] = [
    // { field: "ProductTypeId", headerName: "ID", width: 90 },
    {
  ...
  const handleAddClick = () => {
    navigate("/masterdata/productTypeAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/productTypeAddEdit/${id}`);
    }
  };
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteDataName(params.row.ProductTypeName);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "300", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ProductType"} refresh={OnCallAPI} />
          {Add && (
