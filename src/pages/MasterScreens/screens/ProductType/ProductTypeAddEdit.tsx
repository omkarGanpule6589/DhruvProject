...
import { validation } from "./ProductTypeValidation";
...
const ProductTypeAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.ProductType,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ProductType");
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          if (response.data) {
            ...
            const { ProductTypeName } = result[0] || {};
            initialValues.ProductTypeName = ProductTypeName;
            ...
            setorginalname(result[0].ProductTypeName);
          }
        } catch (error) {
          setformload(false);
          ErrorHandling1(error);
  ...
  const handlePostRequest = async (event) => {
    ...
    const { ProductTypeName, Description } = values;
    const body = {
      MId: 1,
      ProductTypeName,
      Description,
    ...
    console.log(body);
    try {
      const response = await CreateProductType(body);
      if (response.data) {
        setMsg(`${values.ProductTypeName} Created Successfully`);
        ...
        setError(null);
        navigate("/masterdata/producttype");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editProductType(id, values);
      if (response.data) {
        setMsg(`${values.ProductTypeName} Updated Successfully`);
        SuccessNotification(
          `Product Type '${
            values.ProductTypeName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: `odata/ProductType?key=${id}` });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/producttype")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Product Type" : "Edit Product Type "}
        ...
        >
          ...
          >
            ...
            />
            {errors.ProductTypeName && touched.ProductTypeName ? (
              <p className="errorTextColor">{errors.ProductTypeName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="ProductTypeId"
          Bodyname="ProductTypeName"
        />
      )}
    </div>
  );
