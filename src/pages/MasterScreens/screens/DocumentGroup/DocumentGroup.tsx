...
import { getDocumentGroupList } from "./DocumentGroupApi";
...
const DocumentGroup = () => {
  ...
  const [data, setData] = useState<DocumentGroupList[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getDocumentGroupList();
  ...
  const baseColumns: GridColDef[] = [
    {
      ...
      headerName: "Document Group Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/documentGroupAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/documentGroupAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"DocumentGroup"} refresh={OnCallAPI} />
          {Add && (
