//import MuiModules.UIGrid from "@mui/material/MuiModules.UIGrid";
//import  from "@mui/material/MuiModules.UIButton";

import { useLocation, useNavigate, useParams } from "react-router-dom";
import { useFormik } from "formik";
import { validation } from "./EquipmentStatusCodeValidation";
import { useContext, useEffect, useState } from "react";
import {
  CreateEquipmentStatusCode,
  editEquipmentStatusCode,
  getEquipmentStatusById,
} from "./EquipmentStatusCodeAPI";
...
function EquipmentStatusCodeAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.EquipmentStatusCode,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "EquipmentStatusCode");
  ...
  const fetchdata = () => {
    if (id) {
      const fetchEquipmentFamily = async () => {
        setformload(true);
        try {
          const response = await getEquipmentStatusById(id);
          if (response.data) {
            const result = await response.data.value;
            const { EquipmentStatusCode1 } = result[0] || {};
            initialValues.EquipmentStatusCode1 = EquipmentStatusCode1;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setorginalname(result[0].EquipmentStatusCode1);
  ...
  const handlePostRequest = async (event) => {
    ...
    try {
      const response = await CreateEquipmentStatusCode(body);
      if (response.data) {
        setMsg(`${values.EquipmentStatusCode1} Created Successfully`);

        SuccessNotification(
          `Equipment Status Code '${
            values.EquipmentStatusCode1
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/equipmentStatusCode");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editEquipmentStatusCode(id, values);
      if (response.data) {
        setMsg(`${values.EquipmentStatusCode1} Updated Successfully`);
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).equipmentStatusCode 
    });
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/equipmentStatusCode")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Equipment Status Code" : "Edit Equipment Status Code"}
        ...
        >
          ...
          >
            <label htmlFor="EquipmentStatusCode1">
              Equipment Status Code Name <span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="EquipmentStatusCode1"
              id="EquipmentStatusCode1"
              //placeholder="Equipment Status Code"
              value={values.EquipmentStatusCode1}
            ...
            />
            {errors.EquipmentStatusCode1 && touched.EquipmentStatusCode1 ? (
              <p className="errorTextColor">{errors.EquipmentStatusCode1}</p>
            ) : null}
          </MuiModules.UIGrid>
      ...
      {isDeleteCnfDialogOpen && (
        <ConfirmDialog
          ...
          data={deleteData}
          onDelete={OnCallAPI}
          screenName="Equipment Status Code "
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="EquipmentStatusCodeId"
          Bodyname="EquipmentStatusCode"
        />
      )}
    </div>
  );
...
export default EquipmentStatusCodeAddEdit;
