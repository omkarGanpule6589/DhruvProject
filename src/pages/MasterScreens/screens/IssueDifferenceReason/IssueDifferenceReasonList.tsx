...
import { getIssueDiffList } from "./IssueDifferenceReasonApi";
...
function IssueDifferenceReasonList() {
  ...
  const [data, setData] = useState<IssueDiffTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getIssueDiffList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "IssueDifferenceReasonId", headerName: "ID", width: 100 },
    {
      field: "IssueDifferenceReasonName",
      headerName: "Issue Difference Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/issuedifferencereasonAddEdit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/issuedifferencereasonAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"IssueDifferenceReason"} refresh={OnCallAPI} />
          {Add && (
...
export default IssueDifferenceReasonList;
