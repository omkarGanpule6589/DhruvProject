...
import { getSecondAuthList } from "./SecondAuthenticationApi";
...
const SecondAuthentication = () => {
  ...
  const [data, setData] = useState<SecondAuthTypes[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getSecondAuthList();
  ...
  const baseColumns: GridColDef[] = [
    // { field: "SecondAuthenticationId", headerName: "ID", width: 100 },
    {
      field: "SecondAuthentication1",
      headerName: "Second Authentication Name",
      width: 200,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/SecondAuthentication?key=${id}` });
      setDeleteDataName(params.row.SecondAuthentication1);
    },
    []
  );
  const handleAddClick = () => {
    navigate("/masterdata/secondauthenticationAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/secondauthenticationAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"SecondAuthentication"} refresh={OnCallAPI} />
          {Add && (
