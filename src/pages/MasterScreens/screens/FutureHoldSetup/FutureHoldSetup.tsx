...
import { getFutureHoldSetupList } from "./FutureHoldSetupApi";
...
interface FutureHoldSetupTypes {
...
const FutureHoldSetup = () => {
  ...
  const [data, setData] = useState<FutureHoldSetupTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getFutureHoldSetupList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "FutureHoldSetupId", headerName: "ID", width: 100 },
    {
      field: "FutureHoldSetup1",
      headerName: "Future Hold Setup Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/FutureHoldSetupAddEdit/${id}`);
    }
  };
  const handleAddClick = () => {
    navigate("/masterdata/FutureHoldSetupAddEdit");
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"FutureHoldSetup"} refresh={OnCallAPI} />
          {Add && (
...
export default FutureHoldSetup;
