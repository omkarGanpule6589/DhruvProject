...
import { getEsigRoleGroupList } from "./EsigRoleGroupApi";
...
const EsigRoleGroup = () => {
  ...
  const [data, setData] = useState<EsigRoleGroupTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getEsigRoleGroupList();
      debugger
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "SecondAuthenticationRoleGroupId", headerName: "ID", width: 100 },
    {
      field: "SecondAuthenticationRoleGroup1",
      headerName: "SecondAuthentication Role Group Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    if (Read) {
      navigate("/masterdata/esigRoleGroupAddEdit");
    }
  };

  const handleEditClick = (id) => {
    navigate(`/masterdata/esigRoleGroupAddEdit/${id}`);
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "300", width: "100%" }}>
        ...
        >
          <ExportImport Name={"EsigRoleGroup"} refresh={OnCallAPI} />
          {Add && (
        ...
        <GridPro
          rows={data}
          columns={columns}
          id="SecondAuthenticationRoleGroupId"
