...
import { validation } from "./ValidationShippingReason";
import { useContext, useEffect, useState } from "react";

import {
  CreateShippingReason,
  UpdateShippingReason,
  getShippingReasondetailsFetch,
} from "./ShippingReasonApi";
...
const ShippingReasonAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.ShippingReason
      ,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ShippingReason");
  ...
  const handlePostRequest = async (event) => {
    ...
    // const { ShippingReasonName, Description } = values;
    const body = {
      MId: 1,
      ...values,
    ...
    console.log(body);
    try {
      const response = await CreateShippingReason(body);
      if (response.data) {
        setMsg(`${values.ShippingReasonName} Created Successfully`);
        SuccessNotification(
          `Shipping Reason '${
            values.ShippingReasonName
          }' Created Successfully on '${cureenttime()}'`
        );

        setError(null);
        navigate("/masterdata/ShippingReason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await UpdateShippingReason(id, values);
      if (response.data) {
        setMsg(`${values.ShippingReasonName} Updated Successfully`);
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        ...
        try {
          const response = await getShippingReasondetailsFetch(id);

          if (response.data) {
            const result = await response.data.value;
            const { ShippingReasonName } = result[0] || {};
            initialValues.ShippingReasonName = ShippingReasonName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setorginalname(ShippingReasonName);
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).ShippingReason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/ShippingReason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Shipping Reason" : "Edit Shipping Reason"}
        ...
        >
          ...
          >
            <label htmlFor="ShippingReasonName">
              Shipping Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="ShippingReasonName"
              id="ShippingReasonName"
              value={values.ShippingReasonName}
            ...
            />
            {errors.ShippingReasonName && touched.ShippingReasonName ? (
              <p className="errorTextColor">{errors.ShippingReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="ShippingReasonId"
          Bodyname="ShippingReasonName"
        />
      )}
    </div>
  );
...
export default ShippingReasonAddEdit;
