...
import { getShippingReasonList } from "./ShippingReasonApi";
...
const ShippingReason = () => {
  ...
  const [data, setData] = useState<ShippingReason[]>([]);
  ...
  const baseColumns: GridColDef[] = [
    // { field: "ShippingReasonId", headerName: "ID", width: 90 },
    {
      field: "ShippingReasonName",
      headerName: "Shipping Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/shippingReasonAddEdit/${id}`);
    }
  };
  const handleAddClick = () => {
    navigate(`/masterdata/shippingReasonAddEdit`);
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ShippingReason?key=${id}` });
      setDeleteDataName(params.row.ShippingReasonName);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ShippingReason"} refresh={OnCallAPI} />
          {Add && (
