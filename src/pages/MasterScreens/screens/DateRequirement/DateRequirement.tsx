...
import { getDateRequirementList } from "./DateRequirementApi";
...
interface DateReqTypes {
  DateRequirementId: number;
...
function DateRequirement() {
  ...
  const [data, setData] = useState<DateReqTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getDateRequirementList();
      const sortedData = response.data.value.sort((a, b) => {
        return a.DateRequirementName.localeCompare(b.DateRequirementName);
      });
  ...
  const handleAddClick = () => {
    navigate("/masterdata/datereqaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/datereqaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"DateRequirement"} refresh={OnCallAPI} />
          {Add && (
