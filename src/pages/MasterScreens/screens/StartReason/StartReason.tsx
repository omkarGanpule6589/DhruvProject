...
import { getStartReasonList } from "./StartReasonApi";
...
const StartReason = () => {
  ...
  const [data, setData] = useState<StartReason[]>([]);
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "StartReasonId", headerName: "ID", width: 90 },
    {
      field: "StartReasonName",
      headerName: "Start Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/StartReasonAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/StartReasonAddEdit/${id}`);
    }
  };
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/StartReason?key=${id}` });
      setDeleteDataName(params.row.StartReasonName);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"StartReason"} refresh={OnCallAPI} />
          {Add && (
