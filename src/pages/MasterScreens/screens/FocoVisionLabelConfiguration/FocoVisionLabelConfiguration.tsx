...
import { getFocoVisionLabelConfigurationList } from "./FocoVisionLabelConfigurationApi";
...
const FocoVisionLabelConfiguration = () => {
  ...
  const [data, setData] = useState<FactoryTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getFocoVisionLabelConfigurationList();
      debugger
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "FactoryId", headerName: "ID", width: 100 },
    {
      field: "Customer",
     ...
     {
      field: "ItemClass",
      headerName: "ItemClass",
      width: 150,
        valueGetter: (params) => params.row.ItemClass?.ItemClassName || '',
    
    },
     {
      field: "ItemTypeCategory",
      headerName: "Item Type Category",
      width: 150,
        valueGetter: (params) => params.row.ItemTypeCategory?.ItemTypeCategoryName || '',
    
    },
    {
      field: "RequiredColumnsToPrint",
      headerName: "Required Columns To Print",
      width: 250,
    },
    // {
    //   field: "actions",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/focoVisionLabelConfigurationAddEdit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/focoVisionLabelConfigurationAddEdit/${id}`);
    }
  };
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        <MuiModules.UITypography component="h1" variant="h5">
          Foco Vision Label Configuration
        ...
        >
          {/* <ExportImport Name={"Factory"} refresh={OnCallAPI} /> */}
          {Add && (
        ...
        <GridPro
          rows={data}
          columns={columns}
          id="FocoVisionLabelConfigurationId"
...
export default FocoVisionLabelConfiguration;
