...
import { getChangeStateReasonList } from "./ChangeStateReasonApi";
...
interface ChangeStateReasonTypes {
  ChangeStateReasonId: number;
  ChangeStateReasonName: string;
  Description: string;
...
function ChangeStateReason() {
  ...
  const [data, setData] = useState<ChangeStateReasonTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getChangeStateReasonList();
  ...
  const baseColumns: GridColDef[] = [
    {
      field: "ChangeStateReasonName",
      headerName: "Change State Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/changestatereasonaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/changestatereasonaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ChangeStateReason"} refresh={OnCallAPI} />
          {Add && (
...
export default ChangeStateReason;
