...
import { validation } from "./ValidationDepartment";
...
function DepartmentAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({ id, endPoint: Copyendpoints.Department });
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          if (response.data) {
            ...
            const { DepartmentName } = result[0] || {};
            initialValues.DepartmentName = DepartmentName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setorginalname(result[0].DepartmentName);
  ...
  const handlePostRequest = async (event) => {
    ...
    try {
      ...
      if (response.data) {
        setMsg(`${values.DepartmentName} Created Successfully`);
        setError(null);
        SuccessNotification(
          `Department ' ${
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editDepartment(id, values);
      if (response.data) {
        setMsg(`${values.DepartmentName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Department ' ${
            values.DepartmentName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).department 
    });
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Department" : "Edit Department"}
        ...
        >
          ...
          >
            ...
            {errors.DepartmentName && touched.DepartmentName ? (
              <p className="errorTextColor">{errors.DepartmentName}</p>
            ) : null}
          </MuiModules.UIGrid>
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="DepartmentId"
          Bodyname="DepartmentName"
        />
      )}
    </div>
  );
