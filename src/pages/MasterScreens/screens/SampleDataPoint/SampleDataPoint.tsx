...
import { getSampleDataPointList } from "./SampleDataPointApi";
...
interface SampleDataPointTypes {
...
function SampleDataPoint() {
  const navigate = useNavigate();
  const [data, setData] = useState<SampleDataPointTypes[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const sortedData = response.data.value.sort((a, b) => {
        return a.SampleDataPointName.localeCompare(b.SampleDataPointName);
      });
  ...
  const baseColumns: GridColDef[] = [
    // { field: "SampleDataPointId", headerName: "ID", width: 90 },
    {
      field: "SampleDataPointName",
      headerName: "Sample Data Point Name",
      width: 250,
    },
    {
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/Sampledatapoint?key=${id}` });
      setDeleteDataName(params.row.SampleDataPointName);
    },
    []
  );
  const handleAddClick = () => {
    navigate("/masterdata/sampleDataPointAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/sampleDataPointAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: 400, width: "100%" }}>
        ...
        >
          <ExportImport Name={"SampleDataPoint"} refresh={OnCallAPI} />
          {Add && (
...
export default SampleDataPoint;
