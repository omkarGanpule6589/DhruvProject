...
import { getDefectCodeGroupListt } from "./DefectCodeGroupApi";
...
const DefectCodeGroup = () => {
  ...
  const [data, setData] = useState<DefectCodeGroup[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getDefectCodeGroupListt();
  ...
  const baseColumns: GridColDef[] = [
    //  { field: "DefectCodeGroupId", headerName: "ID", width: 100 },
    {
      field: "DefectCodeGroupName",
      headerName: "Defect Code Group Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/DefectCodeGroup?key=${id}` });
      setDeleteDataName(params.row.DefectCodeGroupName);
    },
    []
  );

  const handleAddClick = () => {
    navigate("/masterdata/defectcodegroupaddedit");
  };
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/defectcodegroupaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"DefectCodeGroup"} refresh={OnCallAPI} />
          {Add && (
...
export default DefectCodeGroup;
