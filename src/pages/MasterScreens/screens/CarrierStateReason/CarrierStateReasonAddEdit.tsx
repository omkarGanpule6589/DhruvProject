...
import { validation } from "./validationCarrierStateReason";
import "../../../../App.css";
import { useContext, useEffect, useState } from "react";
import {
  createCarrierStateReason,
  editCarrierStateReason,
  getCarrierStateReasonById,
} from "./CarrierStateReasonApi";
...
export default function CarrierStateReasonAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.CarrierStateReason,
    });
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getCarrierStateReasonById(id);
          if (response.data.value.length > 0) {
            const result = response.data?.value[0];
            (initialValues.CarrierStateReasonName =
              result?.CarrierStateReasonName),
              (initialValues.Description = result?.Description),
              setorginalname(result?.CarrierStateReasonName);
  ...
  const handlePostRequest = async () => {
    ...
    console.log(body);
    try {
      const response = await createCarrierStateReason(body);
      if (response.data) {
        setMsg(`${values.CarrierStateReasonName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Carrier StateReason ' ${
            values.CarrierStateReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/carrierstatereason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editCarrierStateReason(id, values);
      if (response.data) {
        setMsg(`${values.CarrierStateReasonName} Updated Successfully`);
        setError(null);
        SuccessNotification(
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).CarrierStateReason });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/carrierstatereason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Carrier State Reason" : "Edit Carrier State Reason"}
        ...
        >
          ...
          >
            ...
            <label style={{ fontSize: "14px" }}>
              Carrier State Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="CarrierStateReasonName"
              id="CarrierStateReasonName"
              value={values.CarrierStateReasonName}
              onChange={handleChange}
              onBlur={handleBlur}
              autoComplete="off"
            />
            {errors.CarrierStateReasonName && touched.CarrierStateReasonName ? (
              <p className="errorTextColor">{errors.CarrierStateReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="CarrierStateReasonId"
          Bodyname="CarrierStateReasonName"
        />
      )}
    </div>
  );
