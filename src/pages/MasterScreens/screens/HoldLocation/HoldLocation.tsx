...
import { getHoldLocationList } from "./HoldLocationApi";
...
const HoldLocation = () => {
  ...
  const [data, setData] = useState<HoldLocation[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getHoldLocationList();
  ...
  const baseColumns: GridColDef[] = [
    // { field: "HoldLocationId", headerName: "ID", width: 100 },
    {
      field: "HoldLocation1",
      headerName: "Hold Location Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/holdlocationaddedit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/holdlocationaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"HoldLocation"} refresh={OnCallAPI} />
          {Add && (
