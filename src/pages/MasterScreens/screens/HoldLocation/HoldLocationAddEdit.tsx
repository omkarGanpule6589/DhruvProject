...
import {
  CreateHoldLocation,
  EditHoldLocationdetails,
  getHoldLocationDetailFetch,
} from "./HoldLocationApi";
...
const HoldLocationAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({ id, endPoint: Copyendpoints.HoldLocation});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "HoldLocation");
  ...
  const validation = Yup.object({
    HoldLocation1: Yup.string()
      .trim()
      .required("Hold Location Name is required"),
  });
  const initialValues = {
    HoldLocation1: "",
    Description: "",
  ...
  const fetchData = () => {
    if (id) {
      const fetchHoldLocation = async () => {
        setformload(true);
        try {
          const response = await getHoldLocationDetailFetch(id);
          if (response.data.value.length > 0) {
            const result = response.data.value[0];
            (initialValues.HoldLocation1 = result.HoldLocation1),
              (initialValues.Description = result.Description),
              setorginalname(result.HoldLocation1);
  ...
  const handlePostRequest = async () => {
    ...
    try {
      const response = await CreateHoldLocation(body);
      if (response.data) {
        setMsg(`${values.HoldLocation1} Updated Successfully`);

        SuccessNotification(
          `Hold Location '${
            values.HoldLocation1
          }' Created Successfully on '${cureenttime()}'`
        );

        setError(null);
        navigate("/masterdata/holdlocation");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await EditHoldLocationdetails(id, values);
      if (response.data) {
        setMsg(`${values.HoldLocation1} Updated Successfully`);
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).HoldLocation 
    });
      ...
      >
        ...
        <form onSubmit={handleSubmit} onReset={handleReset}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <MuiIcons.ArrowCircleLeftOutlinedIcon
              onClick={() => navigate("/masterdata/holdlocation")}
              style={{ marginRight: "10px" }}
            ></MuiIcons.ArrowCircleLeftOutlinedIcon>
            <MuiModules.UITypography component="h1" variant="h5">
              {!id ? "Add Hold Location" : "Edit Hold Location"}
          ...
          >
            ...
            >
              <label htmlFor="HoldLocation1">
                Hold Location Name<span style={{ color: "red" }}>*</span>
              </label>
              ...
              />
              {errors.HoldLocation1 && touched.HoldLocation1 ? (
                <p className="errorTextColor">{errors.HoldLocation1}</p>
              ) : null}
            </MuiModules.UIGrid>
            <MuiModules.UIGrid
              item
        ...
        {isCopyobjpopupOpen && (
          <ConfirmDialogCopyobj
            ...
            valueRev={copyobjrev}
            Bodyhead="HoldLocationId"
            Bodyname="HoldLocation"
          />
        )}
      </div>
    </>
...
export default HoldLocationAddEdit;
