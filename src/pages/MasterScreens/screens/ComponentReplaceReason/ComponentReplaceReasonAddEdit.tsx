...
import {
  CreateComponentReplaceReasons,
  editComponentReplaceReasons,
  getComponentReplaceReasonsDetails,
} from "./ComponentReplaceReasonApi";
...
const ComponentReplaceReasonAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.ComponentReplaceReason,
    });
  ...
  const validation = Yup.object({
    ComponentReplaceReasonName: Yup.string()
      .trim()
      .required("Component Replace Reason Name is required"),
  });
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getComponentReplaceReasonsDetails(id);
          if (response.data.value.length > 0) {
            const result = await response.data?.value[0];
            initialValues.ComponentReplaceReasonName =
              result?.ComponentReplaceReasonName;
            initialValues.Description = result?.Description;
            setorginalname(result?.ComponentReplaceReasonName);
  ...
  const handlePostRequest = async (event) => {
    ...
    try {
      console.log(body);
      const response = await CreateComponentReplaceReasons(body);
      if (response.data) {
        setMsg(`${values.ComponentReplaceReasonName}Created Successfully`);
        setError(null);
        SuccessNotification(
          `Component Replace Reason  ' ${
            values.ComponentReplaceReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/componentreplacereason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editComponentReplaceReasons(id, values);
      if (response.data) {
        setMsg(`${values.ComponentReplaceReasonName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Component Replace Reason  ' ${
            values.ComponentReplaceReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).ComponentReplaceReasons  });
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/componentreplacereason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id
              ? "Add Component Replace Reason"
              : "Edit Component Replace Reason"}
        ...
        >
          ...
          >
            <label style={{ fontSize: "14px" }}>
              Component Replace Reason Name
              <span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="ComponentReplaceReasonName"
              id="ComponentReplaceReasonName"
              //placeholder="Hold Reason Name"
              value={values.ComponentReplaceReasonName}
            ...
            />
            {errors.ComponentReplaceReasonName &&
            touched.ComponentReplaceReasonName ? (
      ...
      {isDeleteCnfDialogOpen && (
        <ConfirmDialog
          ...
          data={deleteData}
          onDelete={OnCallAPI}
          screenName="Component Replace Reason "
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="ComponentReplaceReasonId"
          Bodyname="ComponentReplaceReasonName"
        />
      )}
    </div>
  );
...
export default ComponentReplaceReasonAddEdit;
