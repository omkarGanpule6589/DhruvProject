...
import { getbonusReasonGroupList } from "./BonusReasonGroupApi";
...
const BonusReasonGroup = () => {
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getbonusReasonGroupList();
  ...
  const baseColumns: GridColDef[] = [
    ...
    {
      field: "GainReasonGroupName",
      headerName: "Gain Reason Group Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handlereadClick = (id) => {
    navigate(`/masterdata/bonusreasongroupaddedit/${id}`);
  };
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      setDeleteCnfDialogOpen(true);
      setDeleteData({ id, endPoint: `odata/bonusReasonGroup?key=${id}` });
      setDeleteDataName(params.row.BuyReasonGroupName);
    },
    []
  );

  const handleAddClick = () => {
    navigate("/masterdata/bonusreasongroupaddedit");
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"GainReasonGroup"} refresh={OnCallAPI} />
          {Add && (
...
export default BonusReasonGroup;
