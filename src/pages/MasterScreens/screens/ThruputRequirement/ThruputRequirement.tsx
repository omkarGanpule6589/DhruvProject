...
import { getThruputRequirementList } from "./ThruputRequirementApi";
...
interface ThruputRequirementTypes {
  ThruputRequirementId: number;
  ThruputRequirement1: string;
  Description: string;
  Revision: string;
...
const ThruputRequirement = () => {
  const navigate = useNavigate();

  const [data, setData] = useState<ThruputRequirementTypes[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getThruputRequirementList();
      const sortedData = response.data.value.sort((a, b) => {
        return a.ThruputRequirement1.localeCompare(b.ThruputRequirement1);
      });
  ...
  const baseColumns: GridColDef[] = [
    // { field: "ThruputRequirementId", headerName: "ID", width: 90 },
    {
      field: "ThruputRequirement1",
      headerName: "Thruput Requirement",
      width: 350,
    },
    {
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ThruputRequirement?key=${id}` });
      setDeleteDataName(params.row.ThruputRequirement1);
    },
    []
  );
  const handleAddClick = () => {
    navigate("/masterdata/thruputRequirementAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/thruputRequirementAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ThruputRequirement"} refresh={OnCallAPI} />
          {Add && (
...
export default ThruputRequirement;
