...
import { getSellReasonList } from "./SellReasonApi";
...
const SellReason = () => {
  ...
  const [data, setData] = useState<SellReason[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "SellReason");
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "SellReasonId", headerName: "ID", width: 90 },
    {
      field: "SellReasonName",
      headerName: "Sell Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/SellReason?key=${id}` });
      setDeleteDataName(params.row.SellReasonName);
    },
    []
  );
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/sellReasonAddEdit/${id}`);
    }
  };
  const handleAddClick = () => {
    navigate(`/masterdata/sellReasonAddEdit`);
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"SellReason"} refresh={OnCallAPI} />
          {Add && (
