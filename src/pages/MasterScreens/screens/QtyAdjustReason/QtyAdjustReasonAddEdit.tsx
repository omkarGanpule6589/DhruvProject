...
import { validation } from "./QtyAdjustReasonValidation";
...
const QtyAdjustReasonAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.QtyAdjustReason,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "QtyAdjustReason");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        ...
        try {
          const response = await getQtyReasonById(id);

          if (response.data) {
            const result = await response.data.value;
            const { QtyAdjustReasonName } = result[0] || {};
            initialValues.QtyAdjustReasonName = QtyAdjustReasonName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setError("");
            setorginalname(result[0].QtyAdjustReasonName);
  ...
  const handlePostRequest = async (event) => {
    ...
    const { QtyAdjustReasonName, Description } = values;
    const body = {
      MId: 1,
      QtyAdjustReasonName,
      Description,
    ...
    console.log(body);
    try {
      const response = await CreateAdjustReason(body);
      if (response.data) {
        setMsg(`${values.QtyAdjustReasonName} Created Successfully`);

        SuccessNotification(
          `Qty Adjust Reason '${
            values.QtyAdjustReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/qtyadjustreason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editQtyAdjustReason(id, values);
      if (response.data) {
        setMsg(`${values.QtyAdjustReasonName} Updated Successfully`);
        SuccessNotification(
          `Qty Adjust Reason '${
            values.QtyAdjustReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).QtyAdjustReason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/qtyadjustreason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Qty Adjust Reason" : "Edit Qty Adjust Reason"}
        ...
        >
          ...
          >
            <label htmlFor="QtyAdjustReasonName">
              Qty Adjust Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="QtyAdjustReasonName"
              id="QtyAdjustReasonName"
              value={values.QtyAdjustReasonName}
            ...
            />
            {errors.QtyAdjustReasonName && touched.QtyAdjustReasonName ? (
              <p className="errorTextColor">{errors.QtyAdjustReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="QtyAdjustReasonId"
          Bodyname="QtyAdjustReasonName"
        />
      )}
    </div>
  );
...
export default QtyAdjustReasonAddEdit;
