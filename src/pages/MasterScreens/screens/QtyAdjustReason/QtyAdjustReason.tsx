...
import { getQtyReasonList } from "./QtyAdjustReasonAPI";
...
const QtyAdjustReason = () => {
  const navigate = useNavigate();
  const [error, setError] = useState<string | null>(null);
  const [qtyAdjustReason, setQtyAdjustReason] = useState<QtyAdjustReason[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getQtyReasonList();
      setQtyAdjustReason(response.data.value);
      setError("");
    } catch (error) {
      setgridload(false);
  ...
  const baseColumns: GridColDef[] = [
    // { field: "QtyAdjustReasonId", headerName: "ID", width: 90 },
    {
      field: "QtyAdjustReasonName",
      headerName: "Qty Adjust Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/qtyAdjustReasonAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/qtyAdjustReasonAddEdit/${id}`);
    }
  };
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/QtyAdjustReason?key=${id}` });
      setDeleteDataName(params.row.QtyAdjustReasonName);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"QtyAdjustReason"} refresh={OnCallAPI} />
          {Add && (
        ...
        <GridPro
          rows={qtyAdjustReason}
          columns={columns}
          id="QtyAdjustReasonId"
...
export default QtyAdjustReason;
