...
import { getOperationDetailList } from "./OperationDetailApi";
...
const OperationDetail = () => {
  ...
  const [data, setData] = useState<OperationDetail[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getOperationDetailList();
      const sortedData = response.data.value.sort((a, b) => {
        return a.OperationDetailName.localeCompare(b.OperationDetailName);
      });
  ...
  const baseColumns: GridColDef[] = [
    //  { field: "OperationDetailId", headerName: "ID", width: 100 },
    {
      field: "OperationDetailName",
      headerName: "Operation Detail Name",
      width: 250,
    },
    {
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/OperationDetail?key=${id}` });
      setDeleteDataName(params.row.OperationDetailName);
    },
    []
  );

  const handleAddClick = () => {
    navigate("/masterdata/operationdetailaddedit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/operationdetailaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"OperationDetail"} refresh={OnCallAPI} />
          {Add && (
