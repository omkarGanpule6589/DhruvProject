...
import { Permissionlist, getPermissionList } from "./PermissionAPI";
...
function Permission() {
  ...
  const baseColumns: GridColDef[] = [
    // { field: "PermissionId", headerName: "ID", width: 90 },
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/Permission?key=${id}` });
      setDeleteDataName(params.row.PermissionName);
    },
    []
  );
  ...
  const handleAddClick = () => {
    navigate("/masterdata/permissionAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/permissionAddEdit/${id}`);
    }
  };
  return (
    <div
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "300", width: "100%" }}>
        ...
        >
          <ExportImport Name={"Permission"} refresh={OnCallAPI} />
          {Add && (
