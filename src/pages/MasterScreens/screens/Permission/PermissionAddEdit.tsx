...
import { validation } from "./ValidationPermission";
import {
  Permissionlist,
  createPermission,
  editPermission,
...
function PermissionAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.Permission,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permissionlist(+RoleId, "Permission");
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          if (response.data) {
            ...
            const { PermissionName } = result[0] || {};
            initialValues.PermissionName = PermissionName;
            const { PermissionType } = result[0] || {};
            initialValues.PermissionType = PermissionType;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setError("");
            setorginalname(result[0].PermissionName);
  ...
  const handlePostRequest = async (event) => {
    ...
    try {
      ...
      if (response.data) {
        setMsg(`${values.PermissionName} Created Successfully`);
        SuccessNotification(
          `Permission '${
        ...
        setError(null);
        navigate("/masterdata/permission");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editPermission(id, values);
      if (response.data) {
        setMsg(`${values.PermissionName} Updated Successfully`);
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).Permission  });
  ...
  //   setorginalname("");
  // };
  const dataPointTypes = ["Modelling", "Transaction"];

  const handlestatus = (event, newValue) => {
    setFieldValue("PermissionType", newValue);
  };
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/permission")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Permission" : "Edit Permission"}
        ...
        >
          ...
          >
            ...
            />
            {errors.PermissionName && touched.PermissionName ? (
              <p className="errorTextColor">{errors.PermissionName}</p>
            ) : null}
          </MuiModules.UIGrid>
          ...
          <MuiModules.UIGrid item xs={12} sm={4}>
            <label style={{ fontSize: "14px" }}>
              Permission Type<span style={{ color: "red" }}>*</span>
            </label>
            ...
            />
            {errors.PermissionType && touched.PermissionType ? (
              <p className="errorTextColor">{errors.PermissionType}</p>
            ) : null}
          </MuiModules.UIGrid>
        </MuiModules.UIGrid>
        {id && (
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="PermissionId"
          Bodyname="PermissionName"
        />
      )}
    </div>
  );
