...
import { getQtyAdjustReasonGroupList } from "./QtyAdjustReasonGroupApi";
...
interface QtyAdjustReasonGroup {
...
const QtyAdjustReasonGroup = () => {
  const navigate = useNavigate();
  const [data, setData] = useState<QtyAdjustReasonGroup[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getQtyAdjustReasonGroupList();
  ...
  const baseColumns: GridColDef[] = [
    // { field: "QtyAdjustReasonGroupId", headerName: "ID", width: 100 },
    {
      field: "QtyAdjustReasonGroupName",
      headerName: "Qty Adjust Reason Group Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/QtyAdjustReasonGroup?key=${id}` });
      setDeleteDataName(params.row.QtyAdjustReasonGroupName);
    },
    []
  );

  const handleAddClick = () => {
    navigate("/masterdata/qtyadjustreasongroupaddedit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/qtyadjustreasongroupaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"QtyAdjustReasonGroup"} refresh={OnCallAPI} />
          {Add && (
...
export default QtyAdjustReasonGroup;
