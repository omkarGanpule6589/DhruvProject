...
import { validation } from "./ValidationReworkReason";
import { useContext, useEffect, useState } from "react";
import {
  editReworkReason,
  createReworkReason,
  getReworkReasonById,
} from "./ReworkReasonAPI";
...
const ReworkReasonAddEdit = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        ...
        try {
          const response = await getReworkReasonById(id);

          if (response.data) {
            const result = await response.data.value;
            const { ReworkReasonName } = result[0] || {};
            initialValues.ReworkReasonName = ReworkReasonName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setError("");
            setorginalname(ReworkReasonName);
  ...
  const handlePostRequest = async (event) => {
    ...
    //const { ReworkReasonName, Description } = values;
    const body = {
      Mid: 1,
      ...values,
    ...
    console.log(body);
    try {
      const response = await createReworkReason(body);
      if (response.data) {
        setMsg(`${values.ReworkReasonName} Created Successfully`);

        SuccessNotification(
          `Rework Reason '${
            values.ReworkReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/reworkreason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editReworkReason(id, values);
      if (response.data) {
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/reworkreason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Rework Reason" : "Edit Rework Reason"}
        ...
        >
          ...
          >
            <label htmlFor="ReworkReasonName">
              Rework Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="ReworkReasonName"
              id="ReworkReasonName"
              value={values.ReworkReasonName}
            ...
            />
            {errors.ReworkReasonName && touched.ReworkReasonName ? (
              <p className="errorTextColor">{errors.ReworkReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
...
export default ReworkReasonAddEdit;
