...
const ReworkReason = () => {
  ...
  const [reworkReason, setReworkReason] = useState<ReworkReason[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ReworkReason");
  ...
  const getReworkReason = async () => {
    ...
    try {
      const response = await getReworkReasonList();
  ...
  const baseColumns: GridColDef[] = [
    ...
    {
      field: "ReworkReasonName",
      headerName: "Rework Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ReworkReason?key=${id}` });
  ...
  const handleAddClick = () => {
    navigate("/masterdata/reworkReasonAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/reworkReasonAddEdit/${id}`);
    }
  };
  return (
    <div
