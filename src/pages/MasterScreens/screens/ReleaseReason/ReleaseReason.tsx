...
import { getRelaseReasonList } from "./ReleaseReasonAPI";
...
const ReleaseReason = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ReleaseReason");
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getRelaseReasonList();
      setReleaseReason(response.data.value);
      setError("");
    } catch (error) {
      setgridload(false);
  ...
  const baseColumns: GridColDef[] = [
    // { field: "ReleaseReasonId", headerName: "ID", width: 90 },
    {
      field: "ReleaseReasonName",
      headerName: "Release Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ReleaseReason?key=${id}` });
      setDeleteDataName(params.row.ReleaseReasonName);
    },
    []
  );
  ...
  const handleAddClick = () => {
    navigate("/masterdata/releaseReasonAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/releaseReasonAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ReleaseReason"} refresh={OnCallAPI} />
          {Add && (
