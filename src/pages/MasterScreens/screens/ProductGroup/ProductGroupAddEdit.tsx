...
import {
  CreateProductGroup,
  EditProductGrouptails,
  getProductGrouplDetailFetch,
...
const ProductGroupAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.ProductGroup,
    });
  ...
  const fetchData = () => {
    if (id) {
      const fetchProductionOrderType = async () => {
        ...
        try {
          const response = await getProductGrouplDetailFetch(id);
          if (response.data.value.length > 0) {
            const result = response.data.value[0];
            (initialValues.ProductGroupName = result.ProductGroupName),
              (initialValues.Description = result.Description);
            setorginalname(result?.ProductGroupName);
  ...
  const handlePostRequest = async () => {
    ...
    try {
      const response = await CreateProductGroup(body);

      if (response.data) {
        setMsg(`${values.ProductGroupName} Updated Successfully`);
        SuccessNotification(
          `Product Group '${
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await EditProductGrouptails(id, values);
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).ProductGroup  });
      ...
      >
        ...
        <form onSubmit={handleSubmit} onReset={handleReset}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <MuiIcons.ArrowCircleLeftOutlinedIcon
              onClick={() => navigate("/masterdata/productgroup")}
              style={{ marginRight: "10px" }}
            ></MuiIcons.ArrowCircleLeftOutlinedIcon>
            <MuiModules.UITypography component="h1" variant="h5">
              {!id ? "Add Product Group " : "Edit Product Group"}
          ...
          >
            ...
            >
              <label htmlFor="ProductGroupName">
                Product Group Name<span style={{ color: "red" }}>*</span>
              </label>
              ...
              />
              {errors.ProductGroupName && touched.ProductGroupName ? (
                <p className="errorTextColor">{errors.ProductGroupName}</p>
              ) : null}
            </MuiModules.UIGrid>
            <MuiModules.UIGrid
              item
        ...
        {isCopyobjpopupOpen && (
          <ConfirmDialogCopyobj
            ...
            valueRev={copyobjrev}
            Bodyhead="ProductGroupId"
            Bodyname="ProductGroupName"
          />
        )}
      </div>
    </>
