...
import { getComponentDefectReasonGroupList } from "./ComponentDefectReasongroupApi";
...
interface ComponentDefectReasonGroup {
...
const ComponentDefectReasongroup = () => {
  ...
  const [data, setData] = useState<ComponentDefectReasonGroup[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getComponentDefectReasonGroupList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "ComponentDefectReasonGroupId", headerName: "ID", width: 100 },
    {
      field: "ComponentDefectReasonGroupName",
      headerName: "Component Defect Reason  Group Name",
      width: 300,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/componentdefectreasongroupaddedit");
  };
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/componentdefectreasongroupaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport
            Name={"ComponentDefectReasonGroup"}
            refresh={OnCallAPI}
          />
          {Add && (
        ...
        <GridPro
          rows={data}
          columns={columns}
          id="ComponentDefectReasonGroupId"
...
export default ComponentDefectReasongroup;
