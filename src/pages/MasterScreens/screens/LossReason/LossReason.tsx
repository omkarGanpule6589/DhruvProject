...
import { validation } from "./validationLossReason";
import { useContext, useEffect, useState } from "react";
import MuiModules from "../../../../MUI-Module/MuiImports";
import MuiIcons from "../../../../MUI-Module/Mui-Icons";
import {
  CreateLossReasondetails,
  editLossReasondetails,
  getLossReasondetailsFetch,
} from "./LossReasonApi";
import {
  ErrorNotification,
  SuccessNotification,
...
const LossReason = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({ id, endPoint: Copyendpoints.LossReason
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "LossReason");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getLossReasondetailsFetch(id);

          if (response.data) {
            const result = await response.data.value;
            const { LossReasonName } = result[0] || {};
            initialValues.LossReasonName = LossReasonName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setorginalname(result[0].LossReasonName);
  ...
  const handlePostRequest = async () => {
    ...
    // const { LossReasonName, Description } = values;
    const body = {
      Mid: 1,
      ...values,
    ...
    console.log(body);
    try {
      const response = await CreateLossReasondetails(body);
      if (response.data) {
        setMsg(`${values.LossReasonName} Created Successfully`);
        setError(null);
        SuccessNotification(
          `Loss Reason ' ${
            values.LossReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/LossReason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editLossReasondetails(id, values);
      if (response.data) {
        setMsg(`${values.LossReasonName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Loss Reason ' ${
            values.LossReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).LossReason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/LossReason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Loss Reason" : "Edit Loss Reason"}
        ...
        >
          ...
          >
            {/* <label htmlFor="LossReasonName"> Loss Reason</label> */}
            <label style={{ fontSize: "14px" }}>
              Loss Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="LossReasonName"
              id="LossReasonName"
              //placeholder="Loss Reason"
              value={values.LossReasonName}
            ...
            />
            {errors.LossReasonName && touched.LossReasonName ? (
              <p className="errorTextColor">{errors.LossReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="LossReasonId"
          Bodyname="LossReasonName"
        />
      )}
    </div>
  );
