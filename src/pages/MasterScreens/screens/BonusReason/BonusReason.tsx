...
import { getBonusReasonList } from "./BonusReasonApi";
...
function BonusReason() {
  ...
  const [data, setData] = useState<BonusReasonTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getBonusReasonList();
  ...
  const handleAddClick = () => {
    navigate("/masterdata/bonusreasonaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/bonusreasonaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"GainReason"} refresh={OnCallAPI} />
          {Add && (
