...
import { validation } from "./validationBounsReason";
import "../../../../App.css";
...
export default function BonusReasonAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({ id, endPoint: Copyendpoints.GainReason });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "GainReason");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getBonusReasonById(id);
          if (response.data.value.length > 0) {
            const result = response.data?.value[0];
            (initialValues.GainReasonName = result?.GainReasonName),
              (initialValues.Description = result?.Description),
              setorginalname(result?.GainReasonName);
  ...
  const handlePostRequest = async () => {
    ...
    const { GainReasonName, Description } = values;
    const body = {
      MId: 1,
      GainReasonName,
      Description,
    ...
    console.log(body);
    try {
      const response = await createBonusReason(body);
      if (response.data) {
        setMsg(`${values.GainReasonName} Created Successfully`);
        setError(null);
        SuccessNotification(
          `Gain Reason ' ${
            values.GainReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/bonusreason");
      } else {
        console.log(error);
        setMsg(null);
      }
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editBonusReason(id, values);
      if (response.data) {
        setMsg(`${values.GainReasonName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Gain Reason  ' ${
            values.GainReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).bonusreason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/bonusreason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Gain Reason " : "Edit Gain Reason"}
        ...
        >
          ...
          >
            {/* <label htmlFor="GainReasonName">Bonus Reason Name</label> */}
            <label style={{ fontSize: "14px" }}>
              Gain Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="GainReasonName"
              id="GainReasonName"
              value={values.GainReasonName}
              onChange={handleChange}
              onBlur={handleBlur}
              autoComplete="off"
            />
            {errors.GainReasonName && touched.GainReasonName ? (
              <p className="errorTextColor">{errors.GainReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="GainReasonId"
          Bodyname="GainReasonName"
        />
      )}
    </div>
  );
