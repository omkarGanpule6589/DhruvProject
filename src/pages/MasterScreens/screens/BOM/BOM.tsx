...
function BOM() {
  ...
  const [data, setData] = useState<BomTypes[]>([]);
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "Bomid", headerName: "ID", width: 90 },
    {
      field: "Bomname",
      headerName: "BOM Name",
      width: 250,
  ...
  const handleAddClick = () => {
    navigate("/masterdata/bomaddedit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/bomaddedit/${id}`);
    }
  };
  return (
    <div
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: 400, width: "100%" }}>
        ...
        >
          <ExportImport Name={"Bom"} refresh={OnCallAPI} />
          {Add && (
