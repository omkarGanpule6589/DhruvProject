...
import { getPrintDefList } from "./PrintLabelAPI";
...
interface PrintLabelDefList {
  PrintLabelDefId: number;
  PrintLabelDefName: string;
  Description: string;
  PrintLabelDefRevision: string;
...
function PrintLabelDef() {
  const navigate = useNavigate();
  const [data, setData] = useState<PrintLabelDefList[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getPrintDefList();
      const sortedData = response.data.value.sort((a, b) => {
        return a.PrintLabelDefName.localeCompare(b.PrintLabelDefName);
      });
  ...
  const baseColumns: GridColDef[] = [
    ///{ field: "PrintLabelDefId", headerName: "ID", width: 90 },

    {
      field: "PrintLabelDefName",
      headerName: "Print Label Def Name",
      width: 200,
    },
    {
    ...
    {
      field: "PrintLabelDefRevision",
      headerName: "Revision",
      width: 150,
    },
    // {
    //   field: "PrintLabelDefRevision",
    //   headerName: "Print Label Def Revision",
    //   width: 250,
    // },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteDataName(params.row.PrintLabelDefName);
    },
    []
  );
  ...
  const handleAddClick = () => {
    navigate("/masterdata/printLabelDefAddEdit");
  };
  const edit = React.useCallback(
    (id: GridRowId, row: GridRowParams<any>) => () => {
      handleEditClick(id);
    },
    []
  );
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/printLabelDefAddEdit/${id}`);
    }
  };
  return (
    <div
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "300", width: "100%" }}>
        ...
        >
          <ExportImport Name={"PrintLabelDef"} refresh={OnCallAPI} />
          {Add && (
