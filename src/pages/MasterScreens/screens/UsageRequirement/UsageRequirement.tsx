...
import { getUsageRequirementList } from "./UsageRequirementApi";
...
interface UsageRequirementTypes {
...
const UsageRequirement = () => {
  ...
  const [data, setData] = useState<UsageRequirementTypes[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getUsageRequirementList();
      const sortedData = response.data.value.sort((a, b) => {
        return a.UsageRequirement1.localeCompare(b.UsageRequirement1);
      });
  ...
  const baseColumns: GridColDef[] = [
    //  { field: "UsageRequirementId", headerName: "Id", width: 100 },
    {
      field: "UsageRequirement1",
      headerName: "Usage Requirement Name",
      width: 250,
    },
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/UsageRequirement?key=${id}` });
      setDeleteDataName(params.row.UsageRequirement1);
    },
    []
  );
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/UsageRequirementAddEdit/${id}`);
    }
  };
  const handleAddClick = () => {
    navigate("/masterdata/UsageRequirementAddEdit");
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"UsageRequirement"} refresh={OnCallAPI} />
          {Add && (
...
export default UsageRequirement;
