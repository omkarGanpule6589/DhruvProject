...
import { getTrainingRequirementGroupList } from "./TrainingRequirementGroupApi";
...
interface TrainingRequirementGroupTypes {
...
const TrainingRequirementGroup = () => {
  const navigate = useNavigate();
  const [data, setData] = useState<TrainingRequirementGroupTypes[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getTrainingRequirementGroupList();
  ...
  const baseColumns: GridColDef[] = [
    //  { field: "TrainingRequirementGroupId", headerName: "ID", width: 90 },
    {
      field: "TrainingRequirementGroup1",
      headerName: "TrainingRequirement Group",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({
        id,
        endPoint: `odata/TrainingRequirementGroup?key=${id}`,
      });
      setDeleteDataName(params.row.TrainingRequirementGroup1);
    },
    []
  );
  const handleAddClick = () => {
    navigate("/masterdata/trainingRequirementgroupAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/trainingRequirementgroupAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"TrainingRequirementGroup"} refresh={OnCallAPI} />
          {Add && (
...
export default TrainingRequirementGroup;
