...
import {
  CreateBuyReasonGroup,
  EditBuyReasonGroupdetails,
  getBuyReasonGroupdetailsFetch,
  getBuyReasonGroupdetailsFetchForEntries,
  getBuyReasonList,
} from "./BuyReasonGroupaPI";
...
interface EquipmentStatusModelType {
  BuyReasonId: number;
  BuyReasonName: string;
}
const Initailrows = [];
const BuyReasonGroupAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.BuyReasonGroup,
    });
  ...
  const handleAddButtonClick = () => {
    const newrow = {
      BuyReasonGroupEntryId: Math.random(),
    };
  ...
  const handelcelledit = (params) => (event, newValue) => {
    ...
    const filteredValue = equipmentStatusModelData.find(
      (item) => item.BuyReasonName === newValue
    );
    const BuyReasonId = filteredValue ? filteredValue.BuyReasonId : null;
    setrows((prevRows) =>
      prevRows.map((row) =>
        row.BuyReasonGroupEntryId === id
          ? { ...row, [field]: value, BuyReasonId: BuyReasonId }
          : row
      )
    );
    fetchoptionsmod(
  ...
  const handleRemoveRow = (id) => {
    setrows((prevRows) =>
      prevRows.filter((row) => row.BuyReasonGroupEntryId !== id)
    );

    if (Number(id) === id && id % 1 == 0) {
      setRowsDeleted((prevRows) => [...prevRows, id]);
    }
    fetchoptionsmod(rows.filter((row) => row.BuyReasonGroupEntryId !== id));
  };
    ...
    try {
      ...
      if (response.data) {
        const filteredRes = res.filter(
          (item) =>
            !tempstore.some(
              (element) => element.BuyReasonId === item.BuyReasonId
            )
        );
  ...
  const fetchoptionsmod = async (tempstore) => {
    try {
      const filteredRes = alloptdata.filter(
        (item) =>
          !tempstore.some((element) => element.BuyReasonId === item.BuyReasonId)
      );
      setEquipmentStatusModelData(filteredRes);
    } catch (error) {
      console.error("Error fetching data:", error);
  ...
  const validation = Yup.object({
    BuyReasonGroupName: Yup.string()
      .trim()
      .required("Buy Reason Group Name is required"),
  });
  const initialValues = {
    BuyReasonGroupName: "",
    Description: "",
  ...
  const fetchData = () => {
    if (id) {
      const fetchBuyReasonGroup = async () => {
        setformload(true);
        try {
          const response = await getBuyReasonGroupdetailsFetch(id);
          if (response.data.value.length > 0) {
            const result = response.data?.value[0];
            (initialValues.BuyReasonGroupName = result?.BuyReasonGroupName),
              (initialValues.Description = result?.Description);
            setorginalname(result?.BuyReasonGroupName);
            setLastModifiedDate(result?.LastModifiedDateTime);
            setLastModifiedUser(result?.LastModifiedUser?.FullName);
            const lists = result.BuyReasonGroupEntries;
            if (lists.length >= 1) {
              const tempstore = [];
              lists.map((item) => {
                const newtemp = {
                  BuyReasonGroupEntryId: item?.BuyReasonGroupEntryId,
                  BuyReasonId: item.BuyReasonId,
                  BuyReasonName: item?.BuyReason?.BuyReasonName,
                };
  ...
  const handlePostRequest = async () => {
    ...
    const body = {
      ...
      BuyReasonGroupEntries: rows
        .map((row) => {
          if (!row.BuyReasonId) {
            return null;
          } else {
            return {
              BuyReasonId: row.BuyReasonId,
    ...
    try {
      const response = await CreateBuyReasonGroup(body);
      if (response.data) {
        setMsg(`${values.BuyReasonGroupName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Buy Reason Group ' ${
            values.BuyReasonGroupName
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/buyreasongroup");
      } else {
    ...
    try {
      ...
      for (let i = 0; i < rowsDeleted.length; i++) {
        requests.push({
          id: `${rowsDeleted[i]}`,
          method: "DELETE",
          url: DeleteSubGridEndPoints(rowsDeleted[i]).BuyReasonGroupEntry,
        });
      }
      const body = {
        requests: requests,
  ...
  const handlePutRequest = async (event) => {
    ...
    const body = {
      ...
      BuyReasonGroupEntries: rows
        .map((row) => {
          if (!row.BuyReasonId) {
            rowsDeleted.push(row.BuyReasonGroupEntryId);
            return null;
          } else {
            if (Number.isInteger(row.BuyReasonGroupEntryId)) {
              return {
                BuyReasonGroupEntryId: row.BuyReasonGroupEntryId,
                BuyReasonId: row.BuyReasonId,
                IsDeleted: false,
                Mid: 1,
              };
    ...
    try {
      const response = await EditBuyReasonGroupdetails(id, body);
      if (response.data) {
        setMsg(`${values.BuyReasonGroupName} Updated Successfully`);
        setError(null);
        SuccessNotification(
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).BuyReasonGroup 
    });
      ...
      >
        ...
        <form onSubmit={handleSubmit} onReset={handleReset}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <MuiIcons.ArrowCircleLeftOutlinedIcon
              onClick={() => navigate("/masterdata/buyreasongroup")}
              style={{ marginRight: "10px" }}
            ></MuiIcons.ArrowCircleLeftOutlinedIcon>
            <MuiModules.UITypography component="h1" variant="h5">
              {!id ? "Add Buy Reason Group " : "Edit Buy Reason Group"}
          ...
          >
            ...
            >
              <label htmlFor="BuyReasonGroupName">
                Buy Reason Group Name <span style={{ color: "red" }}>*</span>
              </label>
              <MuiModules.UITextField
                name="BuyReasonGroupName"
                id="BuyReasonGroupName"
                value={values.BuyReasonGroupName}
              ...
              />
              {errors.BuyReasonGroupName && touched.BuyReasonGroupName ? (
                <p className="errorTextColor">{errors.BuyReasonGroupName}</p>
              ) : null}
            </MuiModules.UIGrid>
            <MuiModules.UIGrid
              item
        ...
        {isCopyobjpopupOpen && (
          <ConfirmDialogCopyobj
            ...
            valueRev={copyobjrev}
            Bodyhead="BuyReasonGroupId"
            Bodyname="BuyReasonGroupName"
          />
        )}
      </div>
    </>
...
export default BuyReasonGroupAddEdit;
