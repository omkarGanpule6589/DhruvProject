...
import { getComponentIssueList } from "./ComponentIssueCodeApi";
...
interface ComponentIssueTypes {
  ComponentIssueCodeId: number;
  ComponentIssueCode1: string;
  Description: string;
...
function ComponentIssueCodeList() {
  ...
  const [data, setData] = useState<ComponentIssueTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getComponentIssueList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "ComponentIssueCodeId", headerName: "ID", width: 100 },
    {
      field: "ComponentIssueCode1",
      headerName: "Component Issue Code Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/componentissuecodeaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/componentissuecodeaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ComponentIssueCode"} refresh={OnCallAPI} />
          {Add && (
...
export default ComponentIssueCodeList;
