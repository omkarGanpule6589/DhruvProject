...
import { getSupplierList } from "./SupplierApi";
...
import { getTestTrialReasonList } from "./TestTrialReasonapi";
...
const TestandTrial = () => {
  ...
  const baseColumns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 250,
      valueGetter: (params) => {
        const dateStr = params.row.ClosureDate;
    
        // Use moment to parse the ClosureDate
        const momentDate = moment(dateStr);
        const today = moment().startOf("day");
    
        if (!momentDate.isValid() || momentDate.isAfter(today)) {
          return "Active";
        } else {
          return "Inactive";
        }
  ...
  const handleAddClick = () => {
    navigate("/masterdata/testandtrialaddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/testandtrialaddEdit/${id}`);
    }
  };
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/Supplier?key=${id}` });
      setDeleteDataName(params.row.Supplier1);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"Supplier"} refresh={OnCallAPI} />
          {Add && (
