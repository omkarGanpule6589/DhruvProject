...
import { getDefectCodeList } from "./DefectCodeApi";
...
function DefectCode() {
  ...
  const [data, setData] = useState<DefectCodeTypes[]>([]);
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/DefectCode?key=${id}` });
      setDeleteDataName(params.row.DefectCodeName);
    },
    []
  );
  ...
  const baseColumns: GridColDef[] = [
    // { field: "DefectCodeId", headerName: "ID", width: 90 },
    {
      field: "DefectCodeName",
      headerName: "Defect Code Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/defectcodeaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/defectcodeaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"DefectCode"} refresh={OnCallAPI} />{" "}
          {Add && (
