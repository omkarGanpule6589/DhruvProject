...
import { getEquipmentFamilyList } from "./EquipmentFamilyApi";
...
interface EquipmentFamilyTypes {
...
function EquipmentFamily() {
  ...
  const [data, setData] = useState<EquipmentFamilyTypes[]>([]);
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/EquipmentFamily?key=${id}` });
      setDeleteDataName(params.row.EquipmentFamilyName);
    },
    []
  );
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getEquipmentFamilyList();
  ...
  const baseColumns: GridColDef[] = [
    // {
    //   field: "EquipmentFamilyId",
    //   headerName: "ID",
    //   width: 100,
    // },
    {
      field: "EquipmentFamilyName",
      headerName: "Equipment Family Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/equipmentfamilyaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/equipmentfamilyaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"EquipmentFamily"} refresh={OnCallAPI} />
          {Add && (
...
export default EquipmentFamily;
