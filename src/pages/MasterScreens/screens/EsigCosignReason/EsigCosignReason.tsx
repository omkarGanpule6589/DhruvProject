...
import { getEsigCosignReasonList } from "./EsigCosignReasonApi";
...
interface SecondAuthenticationReasonTypes {
  SecondAuthenticationCosignReasonId: number;
  SecondAuthenticationCosignReason1: string;
  Description: string;
...
function EsigCosignReason() {
  ...
  const [data, setData] = useState<SecondAuthenticationReasonTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getEsigCosignReasonList();
  ...
  const baseColumns: GridColDef[] = [
    // {
    //   field: "SecondAuthenticationCosignReasonId",
    //   headerName: "ID",
    //   width: 100
    // },
    {
      field: "SecondAuthenticationCosignReason1",
      headerName: "Second Authentication Cosign Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/esigcosignreasonaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/esigcosignreasonaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"EsigCosignReason"} refresh={OnCallAPI} />
          {Add && (
        ...
        <GridPro
          rows={data}
          columns={columns}
          id="SecondAuthenticationCosignReasonId"
...
export default EsigCosignReason;
