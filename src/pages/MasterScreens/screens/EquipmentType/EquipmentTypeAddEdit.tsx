...
import { validation } from "./ValidationEquipmentType";
import "../../../../App.css";
...
export default function EquipmentTypeAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.EquipmentType,
    });
  ...
  const fetchData = () => {
    if (id) {
      const fetchEquipmentType = async () => {
        setformload(true);
        try {
          const response = await getEquipmentTypeById(id);
          if (response.data.value.length > 0) {
            const result = response.data.value[0];
            (initialValues.EquipmentType1 = result.EquipmentType1),
              (initialValues.Description = result.Description),
              (initialValues.EquipmentStatusModelId =
                result.EquipmentStatusModelId),
              setorginalname(result.EquipmentType1);
  ...
  const handlePostRequest = async () => {
    ...
    try {
      const response = await CreateEquipmentType(body);
      if (response.data) {
        setMsg(`${values.EquipmentType1} Created Successfully`);
        setError(null);
        SuccessNotification(
          `Equipment Type ' ${
            values.EquipmentType1
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/equipmenttype");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editEquipmentType(id, values);
      if (response.data) {
        setMsg(`${values.EquipmentType1} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Equipment Type ' ${
            values.EquipmentType1
  ...
  const handleEquipmentStatusModel = (event, newValue) => {
    ...
    setFieldValue(
      ...
      selectedEquipmentStatusModel[0].EquipmentStatusModelId
    );
  };
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).EquipmentType  });
      ...
      >
        ...
        <form onSubmit={handleSubmit} onReset={handleReset}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <MuiIcons.ArrowCircleLeftOutlinedIcon
              onClick={() => navigate("/masterdata/equipmenttype")}
              style={{ marginRight: "10px" }}
            ></MuiIcons.ArrowCircleLeftOutlinedIcon>
            <MuiModules.UITypography component="h1" variant="h5">
              {!id ? "Add Equipment Type" : "Edit Equipment Type"}
          ...
          >
            ...
            >
              <label style={{ fontSize: "14px" }}>
                Equipment Type Name<span style={{ color: "red" }}>*</span>
              </label>

              <MuiModules.UITextField
                name="EquipmentType1"
                id="EquipmentType1"
                value={values.EquipmentType1}
                onChange={handleChange}
                onBlur={handleBlur}
                autoComplete="off"
              />
              {errors.EquipmentType1 && touched.EquipmentType1 ? (
                <p className="errorTextColor">{errors.EquipmentType1}</p>
              ) : null}
            </MuiModules.UIGrid>
            <MuiModules.UIGrid
              item
            ...
            >
              ...
              <MuiModules.UIAutocomplete
                disablePortal
                id="combo-Equipment-Status-Model"
        ...
        {isCopyobjpopupOpen && (
          <ConfirmDialogCopyobj
            ...
            valueRev={copyobjrev}
            Bodyhead="EquipmentTypeId"
            Bodyname="EquipmentType"
          />
        )}
      </div>
    </>
