...
import { getSubstituteReasonList } from "./SubstituteReasonApi";
...
const SubstituteReason = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "SubstituteReason");
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "SubstituteReasonId", headerName: "ID", width: 90 },
    {
      field: "SubstituteReasonName",
      headerName: "Substitute Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/substitutereasonaddedit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/substitutereasonaddedit/${id}`);
    }
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getSubstituteReasonList();
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/Substitutereason?key=${id}` });
      setDeleteDataName(params.row.SubstituteReasonName);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"SubstituteReason"} refresh={OnCallAPI} />
          {Add && (
...
export default SubstituteReason;
