...
import {
  CreateSubstituteReason,
  UpdateSubstituteReason,
  getSubstituteReasonbyid,
} from "./SubstituteReasonApi";
...
const SubstituteReasonAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.SubstituteReason,
    });
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        ...
        try {
          const response = await getSubstituteReasonbyid(id);

          if (response.data) {
            const result = await response.data.value;
            const { SubstituteReasonName } = result[0] || {};
            initialValues.SubstituteReasonName = SubstituteReasonName;
            ...
            setorginalname(SubstituteReasonName);
  ...
  const handlePostRequest = async (event) => {
    ...
    // const { SubstituteReasonName, Description } = values;
    const body = {
      MId: 1,
      ...values,
    ...
    try {
      const response = await CreateSubstituteReason(body);
      if (response.data) {
        setMsg(`${values.SubstituteReasonName} Created Successfully`);
        SuccessNotification(
          `Substitute Reason '${
            values.SubstituteReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/substitutereason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    try {
      console.log(values);
      const response = await UpdateSubstituteReason(id, values);
      if (response.data) {
        setMsg(`${values.SubstituteReasonName} Updated Successfully`);
        SuccessNotification(
          `Substitute Reason '${
            values.SubstituteReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).Substitutereason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/substitutereason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Substitute Reason" : "Edit Substitute Reason"}
        ...
        >
          ...
          >
            <label htmlFor="SubstituteReasonName">
              Substitute Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="SubstituteReasonName"
            ...
            />
            {errors.SubstituteReasonName && touched.SubstituteReasonName ? (
              <p className="errorTextColor">{errors.SubstituteReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="SubstituteReasonId"
          Bodyname="SubstituteReasonName"
        />
      )}
    </div>
  );
...
export default SubstituteReasonAddEdit;
