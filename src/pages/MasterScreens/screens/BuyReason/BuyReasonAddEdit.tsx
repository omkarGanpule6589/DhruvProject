...
import { validation } from "./validationBuyReason";
import "../../../../App.css";
...
export default function AQLLevelAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({ id, endPoint: Copyendpoints.BuyReason });
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getBuyReasonById(id);
          if (response.data.value.length > 0) {
            const result = response.data?.value[0];
            (initialValues.BuyReasonName = result?.BuyReasonName),
              (initialValues.Description = result?.Description),
              setorginalname(result?.BuyReasonName);
  ...
  const handlePostRequest = async (event) => {
    ...
    //const { BuyReasonName, Description } = values;
    const body = {
      MId: 1,
      ...values,
    ...
    console.log(body);
    try {
      const response = await CreateBuyReason(body);
      if (response.data) {
        setMsg(`${values.BuyReasonName} Created Successfully`);
        SuccessNotification(
          `Buy Reason '${
            values.BuyReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/buyreason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editBuyReason(id, values);
      if (response.data) {
        setMsg(`${values.BuyReasonName} Updated Successfully`);
        SuccessNotification(
          `Buy Reason '${
            values.BuyReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).buyreason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/buyreason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Buy Reason" : "Edit Buy Reason"}
        ...
        >
          ...
          >
            <label style={{ fontSize: "14px" }}>
              Buy Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="BuyReasonName"
              id="BuyReasonName"
              value={values.BuyReasonName}
              onChange={handleChange}
              onBlur={handleBlur}
              autoComplete="off"
            />
            {errors.BuyReasonName && touched.BuyReasonName ? (
              <p className="errorTextColor">{errors.BuyReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="BuyReasonId"
          Bodyname="BuyReasonName"
        />
      )}
    </div>
  );
