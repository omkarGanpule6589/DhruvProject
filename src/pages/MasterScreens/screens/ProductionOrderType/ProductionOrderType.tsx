...
import { getProductionOrderTypeList } from "./ProductionOrderTypeAPI";
...
const ProductionOrderType = () => {
  ...
  const [data, setData] = useState<ProductionOrderType[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getProductionOrderTypeList();
  ...
  const baseColumns: GridColDef[] = [
    ...
    {
      field: "ProductionOrderTypeName",
      headerName: "ProductionOrder Type Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ProductionOrderType?key=${id}` });
  ...
  const handleAddClick = () => {
    navigate("/masterdata/productionordertypaddedit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/productionordertypaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ProductionOrderType"} refresh={OnCallAPI} />
          {Add && (
...
export default ProductionOrderType;
