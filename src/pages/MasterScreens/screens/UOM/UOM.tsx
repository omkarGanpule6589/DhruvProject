...
function UOM() {
  ...
  const [data, setData] = useState<UomTypes[]>([]);
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/UOM?key=${id}` });
      setDeleteDataName(params.row.Uomname);
    },
    []
  );

  const handleAddClick = () => {
    navigate("/masterdata/uomaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/uomaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"Uom"} refresh={OnCallAPI} />
          {Add && (
