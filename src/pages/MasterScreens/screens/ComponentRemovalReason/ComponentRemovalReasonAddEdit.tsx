...
import {
  CreateComponentRemovalReason,
  editComponentRemovalReason,
  getComponentRemovalReasonById,
} from "./ComponentRemovalReasonApi";
import {
  ErrorNotification,
  SuccessNotification,
...
import { validation } from "./ComponentRemovalReasonValidation";
...
export default function ComponentRemovalReasonAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.ComponentRemovalReason,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ComponentRemovalReason");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getComponentRemovalReasonById(id);
          if (response.data.value.length > 0) {
            const result = response.data?.value[0];
            (initialValues.CompRemovalReasonName =
              result?.CompRemovalReasonName),
              (initialValues.Description = result?.Description),
              setorginalname(result?.CompRemovalReasonName);
  ...
  const handlePostRequest = async (event) => {
    ...
    console.log(body);
    try {
      const response = await CreateComponentRemovalReason(body);
      if (response.data) {
        setMsg(`${values.CompRemovalReasonName} Created Successfully`);
        SuccessNotification(
          `Component Removal Reason'${
            values.CompRemovalReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/componentremovalreason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editComponentRemovalReason(id, values);
      if (response.data) {
        setMsg(`${values.CompRemovalReasonName} Updated Successfully`);
        SuccessNotification(
          `Component Removal Reason '${
            values.CompRemovalReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).ComponentRemovalReason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/ComponentRemovalReason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id
              ? "Add Component Removal Reason"
              : "Edit Component Removal Reason"}
        ...
        >
          ...
          >
            <label style={{ fontSize: "14px" }}>
              Component Removal Reason Name
              <span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="CompRemovalReasonName"
              id="CompRemovalReasonName"
      ...
      {isDeleteCnfDialogOpen && (
        <ConfirmDialog
          ...
          data={deleteData}
          onDelete={OnCallAPI}
          screenName="Component Removal Reason "
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="ComponentRemovalReasonId"
          Bodyname="ComponentRemovalReasonName"
        />
      )}
    </div>
  );
