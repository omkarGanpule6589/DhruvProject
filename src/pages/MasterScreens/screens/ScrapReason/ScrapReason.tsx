...
import { getScrapReasonList } from "./ScrapReasonApi";
...
interface ScrapReasonTypes {
  SampleDataPointId: string;
...
function ScrapReason() {
  const navigate = useNavigate();
  const [data, setData] = useState<ScrapReasonTypes[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getScrapReasonList();
  ...
  const baseColumns: GridColDef[] = [
    // { field: "ScrapReasonId", headerName: "ID", width: 90 },
    {
      field: "ScrapReasonName",
      headerName: "Scrap Reason Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ScrapReason?key=${id}` });
      setDeleteDataName(params.row.ScrapReasonName);
    },
    []
  );
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/scrapReasonAddEdit/${id}`);
    }
  };
  const handleAddClick = () => {
    navigate(`/masterdata/scrapReasonAddEdit`);
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: 400, width: "100%" }}>
        ...
        >
          <ExportImport Name={"ScrapReason"} refresh={OnCallAPI} />
          {Add && (
