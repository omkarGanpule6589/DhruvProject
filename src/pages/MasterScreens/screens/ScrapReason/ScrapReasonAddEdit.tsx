...
import { validation } from "./ValidationScrapReason";
...
const ScrapReasonAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({ id, endPoint: Copyendpoints.ScrapReason });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "ScrapReason");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        ...
        try {
          const response = await getScrapReasonbyid(id);

          if (response.data) {
            const result = await response.data.value;
            const { ScrapReasonName } = result[0] || {};
            initialValues.ScrapReasonName = ScrapReasonName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setorginalname(ScrapReasonName);
            setError("");
  ...
  const handlePostRequest = async (event) => {
    ...
    const { ScrapReasonName, Description } = values;
    const body = {
      MId: 1,
      ScrapReasonName,
      Description,
    ...
    console.log(body);
    try {
      const response = await CreateScrapReason(body);
      if (response.data) {
        setMsg(`${values.ScrapReasonName} Created Successfully`);

        SuccessNotification(
          `Scrap Reason '${
            values.ScrapReasonName
          }' Created Successfully on '${cureenttime()}'`
        );
        setError(null);
        navigate("/masterdata/ScrapReason");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await UpdateScrapReason(id, values);
      if (response.data) {
        setMsg(`${values.ScrapReasonName} Updated Successfully`);
        SuccessNotification(
          `Scrap Reason '${
            values.ScrapReasonName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).ScrapReason  });
    ...
    >
      ...
      <form onSubmit={handleSubmit} onReset={handleReset}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/ScrapReason")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Scrap Reason" : "Edit Scrap Reason"}
        ...
        >
          ...
          >
            <label htmlFor="ScrapReasonName">
              Scrap Reason Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="ScrapReasonName"
              id="ScrapReasonName"
              value={values.ScrapReasonName}
            ...
            />
            {errors.ScrapReasonName && touched.ScrapReasonName ? (
              <p className="errorTextColor">{errors.ScrapReasonName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="ScrapReasonId"
          Bodyname="ScrapReasonName"
        />
      )}
    </div>
  );
};
export default ScrapReasonAddEdit;
