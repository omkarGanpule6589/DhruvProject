...
import { getDataCollDefList } from "./DataCollectionDefApi";
...
function DataCollectionDef() {
  const navigate = useNavigate();
  const [data, setData] = useState<DataCollDefTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getDataCollDefList();
  ...
  const handleAddClick = () => {
    navigate("/masterdata/datacolldefaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/datacolldefaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"DataCollectionDef"} refresh={OnCallAPI} />
          {Add && (
...
export default DataCollectionDef;
