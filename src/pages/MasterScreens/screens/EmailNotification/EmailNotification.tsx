...
import { getEmailNotificationList } from "./EmailNotificationApi";
...
interface EmailNotificationTypes {
...
function EmailNotification() {
  ...
  const [data, setData] = useState<EmailNotificationTypes[]>([]);
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/EmailNotification?key=${id}` });
      setDeleteDataName(params.row.EmailNotification1);
    },
    []
  );
  ...
  const baseColumns: GridColDef[] = [
    ...
    // },
    {
      field: "EmailNotification1",
      headerName: "Email Notification Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/emailnotificationaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/emailnotificationaddedit/${id}`);
    }
  };
