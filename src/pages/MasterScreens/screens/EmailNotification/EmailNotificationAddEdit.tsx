...
import { validation } from "./ValidationEmailNotification";
import "../../../../App.css";
import { useState, useEffect, useContext } from "react";
import {
  CreateEmailNotification,
  editEmailNotification,
  getEmailNotificationdetailsFetch,
} from "./EmailNotificationApi";
...
export default function EmailNotificationAddEdit() {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.EmailNotification,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "EmailNotification");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getEmailNotificationdetailsFetch(id);
          if (response.data) {
            const result = await response.data.value;
            const { EmailNotification1 } = result[0] || {};
            initialValues.EmailNotification1 = EmailNotification1;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            const { Sender } = result[0] || {};
            initialValues.Sender = Sender;
            const { Recipient } = result[0] || {};
            initialValues.Recipient = Recipient;
            setorginalname(result[0].EmailNotification1);
  ...
  const handlePutRequest = async (event) => {
    ...
    try {
      const response = await editEmailNotification(id, values);
      if (response.data) {
        setMsg(`${values.EmailNotification1} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Email Notification ' ${
            values.EmailNotification1
          }' Updated Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/EmailNotification");
      } else {
  ...
  const handlePostRequest = async (event) => {
    ...
    console.log(body);
    try {
      const response = await CreateEmailNotification(body);
      if (response.data) {
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).EmailNotification });
      ...
      >
        ...
        <form onSubmit={handleSubmit} onReset={handleReset}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <MuiIcons.ArrowCircleLeftOutlinedIcon
              onClick={() => navigate("/masterdata/EmailNotification")}
              style={{ marginRight: "10px" }}
            ></MuiIcons.ArrowCircleLeftOutlinedIcon>
            <MuiModules.UITypography component="h1" variant="h5">
              {!id ? "Add Email Notification" : "Edit Email Notification"}
          ...
          >
            ...
            >
              <label style={{ fontSize: "14px" }}>
                Email Notification Name<span style={{ color: "red" }}>*</span>
              </label>

              <MuiModules.UITextField
                name="EmailNotification1"
                ...
                autoComplete="off"
              />
              {errors.EmailNotification1 && touched.EmailNotification1 ? (
                <p className="errorTextColor">{errors.EmailNotification1}</p>
              ) : null}
            </MuiModules.UIGrid>
            <MuiModules.UIGrid
              item
        ...
        {isCopyobjpopupOpen && (
          <ConfirmDialogCopyobj
            ...
            valueRev={copyobjrev}
            Bodyhead="EmailNotificationId"
            Bodyname="EmailNotificationName"
          />
        )}
      </div>
    </>
