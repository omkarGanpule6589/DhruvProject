...
import { validation } from "./ValidationInspectionLevel";
import { useState, useEffect, useContext } from "react";
import {
  getInspectionLevelDetails,
  editInspectionLevel,
  CreateInspectionLevel,
} from "./InspectionLevelApi";
...
const InspectionLevelAddEdit = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.InspectionLevel,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "InspectionLevel");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getInspectionLevelDetails(id);
          if (response.data.value.length > 0) {
            const result = await response.data?.value[0];
            initialValues.InspectionLevelName = result?.InspectionLevelName;
            initialValues.Description = result?.Description;
            setorginalname(result?.InspectionLevelName);
  ...
  const handlePostRequest = async (event) => {
    ...
    //const { InspectionLevelName, Description } = values;
    const body = {
      Mid: 1,
      ...values,
    ...
    try {
      const response = await CreateInspectionLevel(body);
      if (response.data) {
        setMsg(`${values.InspectionLevelName}Created Successfully`);
        setError(null);
        SuccessNotification(
          `Inspection Level' ${
            values.InspectionLevelName
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/inspectionlevel");
      } else {
        console.log(error);
        //setError(`Errror Adding Data. Please check the server`);
        setMsg(null);
      }
    } catch (error) {
      setSaveload(false);
  ...
  const handlePutRequest = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await editInspectionLevel(id, values);
      if (response.data) {
        setMsg(`${values.InspectionLevelName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Inspection Level' ${
            values.InspectionLevelName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).inspectionlevel  });
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/inspectionlevel")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Inspection Level" : "Edit Inspection Level"}
        ...
        >
          ...
          >
            <label style={{ fontSize: "14px" }}>
              Inspection Level Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="InspectionLevelName"
              id="InspectionLevelName"
              //placeholder="InspectionLevelName"
              value={values.InspectionLevelName}
              onChange={handleChange}
              onBlur={handleBlur}
              autoComplete="off"
            />
            {errors.InspectionLevelName && touched.InspectionLevelName ? (
              <p className="errorTextColor">{errors.InspectionLevelName}</p>
            ) : null}
          </MuiModules.UIGrid>
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="InspectionLevelId"
          Bodyname="InspectionLevelName"
        />
      )}
    </div>
  );
...
export default InspectionLevelAddEdit;
