...
import { getInspectionLevelList } from "./InspectionLevelApi";
...
interface InspectionLevelTypes {
...
const InspectionLevel = () => {
  ...
  const [data, setData] = useState<InspectionLevelTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getInspectionLevelList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "InspectionLevelId", headerName: "ID", width: 100 },
    {
      field: "InspectionLevelName",
      headerName: "Inspection Level Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/inspectionLevelAddEdit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/inspectionLevelAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"InspectionLevel"} refresh={OnCallAPI} />
          {Add && (
