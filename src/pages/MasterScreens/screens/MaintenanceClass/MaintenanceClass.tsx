...
import { validation } from "./validationMaintenanceClass";
import { useContext, useEffect, useState } from "react";
import {
  CreateMaintenanceClassdetails,
  editMaintenanceClassdetails,
  getMaintenanceClassdetailsFetch,
} from "./MaintenanceClassApi";
...
const MaintenanceClass = () => {
  ...
  const Copyobjclk = (event) => {
    ...
    setcopyobjdata({
      id,
      endPoint: Copyendpoints.MaintenanceGroup
      ,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Permission(+RoleId, "MaintenanceGroup");
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setformload(true);
        try {
          const response = await getMaintenanceClassdetailsFetch(id);

          if (response.data) {
            const result = await response.data.value;
            const { MaintenanceGroupName } = result[0] || {};
            initialValues.MaintenanceGroupName = MaintenanceGroupName;
            const { Description } = result[0] || {};
            initialValues.Description = Description;
            setorginalname(result[0].MaintenanceGroupName);
  ...
  const handlePostRequest = async (event) => {
    ...
    //const { MaintenanceGroupName, Description } = values;
    const body = {
      Mid: 1,
      ...values,
    ...
    debugger
    try {
      const response = await CreateMaintenanceClassdetails(body);
      if (response.data) {
        setMsg(`${values.MaintenanceGroupName} Created Successfully`);
        setError(null);
        SuccessNotification(
          `Maintenance Group' ${
            values.MaintenanceGroupName
          }' Created Successfully on '${cureenttime()}'`
        );
        navigate("/masterdata/MaintenanceClass");
      } else {
  ...
  const handlePutRequest = async (event) => {
    ...
    try {
      const response = await editMaintenanceClassdetails(id, values);
      debugger
      if (response.data) {
        setMsg(`${values.MaintenanceGroupName} Updated Successfully`);
        setError(null);
        SuccessNotification(
          `Maintenance Group' ${
            values.MaintenanceGroupName
  ...
  const deleteCnf = (event) => {
    ...
    setDeleteData({ id, endPoint: deleteendponts(id).MaintenanceGroupdelete 
    });
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <MuiIcons.ArrowCircleLeftOutlinedIcon
            onClick={() => navigate("/masterdata/MaintenanceClass")}
            style={{ marginRight: "10px" }}
          ></MuiIcons.ArrowCircleLeftOutlinedIcon>
          <MuiModules.UITypography component="h1" variant="h5">
            {!id ? "Add Maintenance Group" : "Edit Maintenance Group"}
        ...
        >
          ...
          >
            ...
            <label style={{ fontSize: "14px" }}>
              Maintenance Group Name<span style={{ color: "red" }}>*</span>
            </label>
            <MuiModules.UITextField
              name="MaintenanceGroupName"
              id="MaintenanceGroupName"
              //placeholder="Maintenance Class"
              value={values.MaintenanceGroupName}
            ...
            />
            {errors.MaintenanceGroupName && touched.MaintenanceGroupName ? (
              <p className="errorTextColor">{errors.MaintenanceGroupName}</p>
            ) : null}
          </MuiModules.UIGrid>
          <MuiModules.UIGrid
            item
      ...
      {isCopyobjpopupOpen && (
        <ConfirmDialogCopyobj
          ...
          valueRev={copyobjrev}
          Bodyhead="MaintenanceGroupId"
          Bodyname="MaintenanceGroup"
        />
      )}
    </div>
  );
