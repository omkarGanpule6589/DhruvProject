...
import { getMaintenanceClassList } from "./MaintenanceClassApi";
...
function MaintenanceClassList() {
  ...
  const [data, setData] = useState<MaintenanceClassTypes[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getMaintenanceClassList();
  ...
  const baseColumns: GridColDef[] = [
    //{ field: "MaintenanceGroupId", headerName: "ID", width: 100 },
    {
      field: "MaintenanceGroupName",
      headerName: "Maintenance Group Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleAddClick = () => {
    navigate("/masterdata/maintenanceClassPage");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/maintenanceClassPage/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <Box sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"MaintenanceClass"} refresh={OnCallAPI} />
          {Add && (
...
export default MaintenanceClassList;
