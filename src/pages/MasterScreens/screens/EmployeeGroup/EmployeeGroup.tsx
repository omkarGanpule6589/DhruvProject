...
import { getEmployeeGroupList } from "./EmployeeGroupApi";
...
const EmployeeGroup = () => {
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getEmployeeGroupList();
  ...
  const baseColumns: GridColDef[] = [
    // { field: "EmployeeGroupId", headerName: "ID", width: 100 },
    {
  ...
  const handleAddClick = () => {
    navigate("/masterdata/employeegroupaddedit");
  };

  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/employeegroupaddedit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"EmployeeGroup"} refresh={OnCallAPI} />
          {Add && (
