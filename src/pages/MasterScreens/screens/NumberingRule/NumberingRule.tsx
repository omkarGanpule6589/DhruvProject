...
import { getNumberingruleList } from "./NumberingRuleApi";
...
const NumberingRule = () => {
  ...
  const [data, setData] = useState<NumberingRule[]>([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getNumberingruleList();
  ...
  const baseColumns: GridColDef[] = [
    // { field: "NumberingRuleId", headerName: "ID", width: 100 },
    {
      field: "NumberingRuleName",
      headerName: "Numbering Rule Name",
      width: 200,
    },
    {
      field: "Description",
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/NumberingRule?key=${id}` });
      setDeleteDataName(params.row.NumberingRuleName);
    },
    []
  );

  const handleAddClick = () => {
    navigate("/masterdata/numberingRuleAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/numberingRuleAddEdit/${id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"NumberingRule"} refresh={OnCallAPI} />
          {Add && (
