...
import { getShippingReasonGroupList } from "./ShippingReasonGroupApi";
...
interface ShippingReasonGroup {
  ShippingReasonGroupId: number;
  ShippingReasonGroupName: string;
  Description: string;
...
const ShippingReasonGroup = () => {
  const navigate = useNavigate();
  const [data, setData] = useState<ShippingReasonGroup[]>([]);
  ...
  const baseColumns: GridColDef[] = [
    // { field: "ShippingReasonGroupId", headerName: "ID", width: 90 },
    {
      field: "ShippingReasonGroupName",
      headerName: "Shipping Reason Group Name",
      width: 250,
    },
    {
      field: "Description",
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/shippingreasongroupaddedit/${id}`);
    }
  };
  const handleAddClick = () => {
    navigate(`/masterdata/shippingreasongroupaddedit`);
    // Add any additional logic to perform on "Add" button click
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await getShippingReasonGroupList();
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({ id, endPoint: `odata/ShippingReasonGroup?key=${id}` });
      setDeleteDataName(params.row.ShippingReasonGroupName);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"ShippingReasonGroup"} refresh={OnCallAPI} />
          {Add && (
...
export default ShippingReasonGroup;
