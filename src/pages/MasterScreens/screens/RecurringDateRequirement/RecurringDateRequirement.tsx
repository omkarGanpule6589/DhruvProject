...
import { getRecurringDateList } from "./RecurringDateRequirementAPI";
...
const RecurringDateRequirement = () => {
  const [recurringDateRequirement, setRecurringDateRequirement] = useState<
    RecurringDateRequirement[]
  >([]);
  ...
  const fetchData = async () => {
    setgridload(true);
    try {
      const response = await getRecurringDateList();
      const sortedData = response.data.value.sort((a, b) => {
        return a.RecurringDateRequirement1.localeCompare(
          b.RecurringDateRequirement1
        );
      });

      setRecurringDateRequirement(sortedData);
      //setRecurringDateRequirement(response.data.value);
      setError("");
    } catch (error) {
      setgridload(false);
  ...
  const baseColumns: GridColDef[] = [
    // { field: "RecurringDateRequirementId", headerName: "ID", width: 90 },
    {
      field: "RecurringDateRequirement1",
      headerName: "Recurring Date Requirement Name",
      width: 250,
    },
    {
  ...
  const handleAddClick = () => {
    navigate("/masterdata/recurringDateRequirementAddEdit");
  };
  ...
  const handleEditClick = (id) => {
    if (Read) {
      navigate(`/masterdata/recurringDateRequirementAddEdit/${id}`);
    }
  };
  ...
  const deleteCnf = React.useCallback(
    (id: GridRowId, params) => () => {
      ...
      setDeleteData({
        id,
        endPoint: `odata/RecurringDateRequirement?key=${id}`,
      });
      setDeleteDataName(params.row.RecurringDateRequirement1);
    },
    []
  );
  ...
  return (
    ...
    >
      ...
      <MuiModules.UIBox sx={{ height: "400", width: "100%" }}>
        ...
        >
          <ExportImport Name={"RecurringDateRequirement"} refresh={OnCallAPI} />
          {Add && (
        ...
        <GridPro
          rows={recurringDateRequirement}
          columns={columns}
          id="RecurringDateRequirementId"
...
export default RecurringDateRequirement;
